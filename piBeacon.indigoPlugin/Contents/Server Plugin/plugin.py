...
_GlobalConst_allowedCommands       = [u"up", u"down", u"pulseUp", u"pulseDown", u"continuousUpDown", u"analogWrite", u"disable", u"newMessage", u"startCalibration", u"rampUp", u"rampDown", u"rampUpDown"]  # commands support for GPIO pins
_GlobalConst_allowedSensors        = [u"ultrasoundDistance", u"vl503l0xDistance", u"vl6180xDistance", u"vcnl4010Distance", # dist / light
                         ...
                         u"i2cTCS34725", u"i2cTSL2561", u"i2cVEML6070", u"i2cVEML6030", u"i2cVEML6040",     # light 
                         u"i2cVEML6075", u"i2cIS1145", u"i2cOPT3001",                                   # light   
                         u"BLEsensor",
                         u"Wire18B20", u"i2cTMP102", u"i2cMCP9808", u"i2cLM35A",                         # temp 
                         u"DHTxx", u"DHT11", u"i2cAM2320", u"i2cSHT21",                                   # temp / hum
...
_GlobalConst_allowedpiSends        = [u"updateParams", u"resetGPIOCount", u"reboot", u"resetOutput", u"updateAllFiles", u"shutdown", u"getStats", u"upgradeOpSys"]
...
####-------------------------------------------------------------------------####
    def __init__(self, pluginId, pluginDisplayName, pluginVersion, pluginPrefs):
        ...
        self.indigoVersion      = indigo.server.version.split(".")[0]
...
####-------------------------------------------------------------------------####
    def startup(self):
        try:
            ...
            self.getDebugLevels()
            #if self.ML.decideMyLog(u"Special"): self.ML.myLog( text =  u"memory used: "+str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / (1024.*1024.))[:6]+"[MB] @ startup   ")
            ...
            if self.logFileActive!="no" : self.ML.myLog( text = u" --V " + self.pluginVersion + u"   initializing ")
...
####-------------------------------------------------------------------------####
    def initFileDir(self):
            ...
            if not os.path.exists(self.piDirIndigo):
                os.mkdir(self.piDirIndigo)

    
            if not os.path.exists(self.piDir):
                os.mkdir(self.piDir)
            if not os.path.exists(self.piDir):
                self.errorLog(u"error creating the plugin data dir did not work, can not create: "+ self.piDir)
                self.sleep(1000)
                exit()
                
            if os.path.exists(self.piDirOld + "all") and not os.path.exists(self.piDir + "all"):
                indigo.server.log(u" moving "+ "cp -R" + self.piDirOld+"* " + self.piDir )
                os.system(u"cp -R " + self.piDirOld+"* " + self.piDir )

            if not os.path.exists(self.piDir+"plotPositions"):
                os.mkdir(self.piDir+"plotPositions")
...
####-------------------------------------------------------------------------####
    def setupBasicFiles(self):
        try:
            ...
            if not os.path.exists(self.piDir + u"all"):
                os.mkdir(self.piDir + u"all")
            if not os.path.exists(self.piDir + u"rejected"):
                os.mkdir(self.piDir + u"rejected")
                os.system(u" mv " + self.piDir + u"rejct* " + self.piDir + u"rejected")
            if not os.path.exists(self.piDir + u"interfaceFiles"):
                os.mkdir(self.piDir + u"interfaceFiles")
                os.system(u"rm " + self.piDir + u"param*")
                os.system(u"rm " + self.piDir + u"interfa*")
                os.system(u"rm " + self.piDir + u"wpa_supplicant*")
            if not os.path.exists(self.piDir + u"soundFiles"):
                os.mkdir(self.piDir + u"soundFiles")
            if not os.path.exists(self.piDir + u"displayFiles"):
                os.mkdir(self.piDir + u"displayFiles")
            if not os.path.exists(self.cameraImagesDir):
                os.mkdir(self.cameraImagesDir)
    ...
    def startupFIXES0(self): # change old names used
        ...
        try:
            for dev in indigo.devices.iter(self.pluginId):
                ...
                if dev.deviceTypeId == u"beacon" or dev.deviceTypeId.find("rPI")> -1 or dev.deviceTypeId == u"BLEconnect": 
                    try:
...
####-------------------------------------------------------------------------####
    def getDebugLevels(self):
        try:
            self.debugLevel         = []
            if self.pluginPrefs.get(u"debugLogic", False):        self.debugLevel.append(u"Logic")
            if self.pluginPrefs.get(u"debugDevMgmt", False):      self.debugLevel.append(u"DevMgmt")
            if self.pluginPrefs.get(u"debugBeaconData", False):   self.debugLevel.append(u"BeaconData")
            if self.pluginPrefs.get(u"debugSensorData", False):   self.debugLevel.append(u"SensorData")
            if self.pluginPrefs.get(u"debugOutputDevice", False): self.debugLevel.append(u"OutputDevice")
            if self.pluginPrefs.get(u"debugUpdateRPI", False):    self.debugLevel.append(u"UpdateRPI")
            ...
            if self.pluginPrefs.get(u"debugSocket", False):       self.debugLevel.append(u"Socket")
            self.debugPlotPositions=                              self.pluginPrefs.get(u"debugPlotPositions", False)
            if self.debugPlotPositions:                           self.debugLevel.append(u"PlotPositions")
...
####-------------------------------------------------------------------------####
    def setVariables(self):
        try:
            ...
            self.piDirIndigo                = self.MAChome + "/indigo/"
            self.piDir                      = self.piDirIndigo + u"piBeacon/"
            self.piDirOld                   = self.MAChome + u"/documents/piBeacon/"
            self.cameraImagesDir            = self.piDir+"cameraImages/"
            ...
            if self.logFileActive =="no":
                ...
                self.logFileName = self.indigoPath.split("Plugins/")[0]+"logs/"+self.pluginId+"/piBeacon.log"
                indigo.server.log("logfile handling to : "+self.logFileName)
            else:
                self.logFileName    = self.piDir + "piBeacon.log"
                indigo.server.log("logfile handling to : "+self.logFileName)
            
            self.debugLevel                 = []
            self.maxFileSizeLogFile         = 5000000
            self.lastCheckLogFileSize       = time.time()
...
####-------------------------------------------------------------------------####
    def readTcpipSocketStats(self):
        self.dataStats ={}
        try:
            f = open(self.piDir + "dataStats", u"r")
...
####-------------------------------------------------------------------------####
    def saveTcpipSocketStats(self):
         self.writeJson(self.dataStats, fName=self.piDir + u"dataStats", format=True )
...
####-------------------------------------------------------------------------####
    def saveCARS(self,force=False):
        ...
        self.writeJson(self.CARS,fName=self.piDir + u"CARS" )

####-------------------------------------------------------------------------####
    def readCARS(self):
        try:
            f = open(self.piDir + "CARS", u"r")
    ...
    def updateCARS(self,beacon,beaconDev,beaconNewStates,force=False):
        try:
            ...
            if  oldCarStatus == "away":
                ...
                elif indigoCarIds in self.updateStatesDict and "location" in self.updateStatesDict[indigoCarIds]["keys"] and self.updateStatesDict[indigoCarIds]["keys"][u"location"][0] == "home":
            ...
            if u"location" in whatForStatus: st =      self.updateStatesDict[indigoCarIds]["keys"][u"location"][0]
            if u"engine"   in whatForStatus: st +="/"+ self.updateStatesDict[indigoCarIds]["keys"][u"engine"][0]
            if u"motion"   in whatForStatus: st +="/"+ self.updateStatesDict[indigoCarIds]["keys"][u"motion"][0]
            st = st.strip(u"/").strip(u"/")
            self.addToStatesUpdateDict(indigoCarIds,"status",st)
            if self.updateStatesDict[indigoCarIds]["keys"][u"location"][0] ==u"home":
...
####-------------------------------------------------------------------------####
    def setupCARS(self,carIdi,props,mode=""):
        ...
        try:
            ...
            if update: 
                self.setALLrPiV(u"piUpToDate", [u"updateParams"])
        except  Exception, e:
            self.ML.myLog( text = u"setupCARS in Line '%s:'%s'" % (sys.exc_traceback.tb_lineno, e))
            return props
        return  props
...
####-------------------------------------------------------------------------####
    def readConfig(self):  ## only once at startup
        try:
            ...
            self.RPI = self.getParamsFromFile(self.piDir+"RPIconf")
            if self.RPI =={}: 
                self.ML.myLog( text = self.piDir + "RPIconf file does not exist or has bad data, will do a new setup ")


            self.sensorMessages = self.getParamsFromFile(self.piDir+ "sensorMessages")
            ...
            for pi in range(_GlobalConst_numberOfRPI):
                if self.RPI[unicode(pi)][u"piOnOff"] == "0": 
                    ...
                    self.removeONErPiV(pi, u"piUpToDate", [u"updateParams", u"updateAllFiles"])


            self.beacons           = self.getParamsFromFile(self.piDir+ "beacons")
            ...
            self.beaconsUUIDtoName   = self.getParamsFromFile(self.piDir+"beaconsUUIDtoName",     oldName= self.piDir+"UUIDtoName")
            self.beaconsUUIDtoIphone = self.getParamsFromFile(self.piDir+"beaconsUUIDtoIphone",   oldName= self.piDir+"UUIDtoIphone")
            self.beaconsIgnoreUUID   = self.getParamsFromFile(self.piDir+"beaconsIgnoreUUID",     oldName= self.piDir+"beaconsIgnoreFamily")
            self.rejectedByPi        = self.getParamsFromFile(self.piDir+"rejected/rejectedByPi.json")
            self.version             = self.getParamsFromFile(self.piDir+"dataVersion", default=0)



            if dataVersion != self.version:
                self.version="updateNeeded"
...
####-------------------------------------------------------------------------####
    def savebeaconPositionsFile(self):
        ...
        try:
                f = open(self.piDir + "plotPositions/positions.json", u"w")
...
####-------------------------------------------------------------------------####
    def setImageParameters(self):
        self.beaconPositionsData[u"piDir"]          = self.piDir+"plotPositions"
        self.beaconPositionsData[u"logLevel"]       = self.debugPlotPositions
        self.beaconPositionsData[u"logFile"]        = self.piDir+"plotPositions/plotPositions.log"
    ...
    def calcPitoPidist(self):
        ...
        for ii in range(_GlobalConst_numberOfiBeaconRPI):
            try:
                ...
                try: 
                    Pii = propsii[u"PosXYZ"].split(u",")
                    if len(Pii) != 3: continue
                    Pii = [float(Pii[0]),float(Pii[1]),float(Pii[2])]
                ...
                for jj in range(ii+1, _GlobalConst_numberOfiBeaconRPI):
                    try:
                        ...
                        try: 
                            Pjj = propsjj[u"PosXYZ"].split(u",")
                            if len(Pjj) != 3: continue
                            Pjj = [float(Pjj[0]),float(Pjj[1]),float(Pjj[2])]
                        except  Exception, e:
                            self.ML.myLog( text =  u"calcPitoPidist in Line '%s' has error='%s'" % (sys.exc_traceback.tb_lineno, e))
                            continue
                        deltaDist =0
...
####-------------------------------------------------------------------------####
    def fixConfig(self,checkOnly = ["all"],fromPGM=""):
        ...
        if time.time() - self.lastFixConfig < 10: return
            ...
            if "rpi" in checkOnly :
                for pi in range(_GlobalConst_numberOfRPI):
                    if self.RPI[unicode(pi)][u"ipNumberPi"] != "":
                        self.RPI[unicode(pi)][u"ipNumberPiSendTo"] = copy.deepcopy(self.RPI[unicode(pi)][u"ipNumberPi"])
        ...
        try:
            if "all" in checkOnly:
                delDEV = []
                for dev in indigo.devices.iter(self.pluginId+",props.isRPIDevice,props.isRPISensorDevice,props.isBeaconDevice,props.isCARDevice,props.isBLEconnectDevice"):
        ...
        try:
            ...
            self.ML.myLog( text =  u"fixConfig in Line '%s' has error='%s'" % (sys.exc_traceback.tb_lineno, e))
        #self.ML.myLog( text =  u"fixConfig leaving after all  %4.2f secs"%(time.time()-self.lastFixConfig) )
        ...
        #self.ML.myLog( text =  u"fixConfig leaving after 3  %4.2f secs"%(time.time()-self.lastFixConfig) )
        ...
        if "all" in checkOnly:
            self.syncSensors()
        #self.ML.myLog( text =  u"fixConfig leaving after 5  %4.2f secs"%(time.time()-self.lastFixConfig) )

        self.saveConfig()
        #self.ML.myLog( text =  u"fixConfig leaving end  %4.2f secs"%(time.time()-self.lastFixConfig) )
        return
    ...
    def saveSensorMessages(self,devId="",item="", value=0):
        try:
            if devId != "":
                ...
                self.writeJson(self.sensorMessages,fName=self.piDir + u"sensorMessages")
            return
        except  Exception, e:
...
####-------------------------------------------------------------------------####
    def saveConfig(self):
        self.writeJson(self.RPI, fName=self.piDir + u"RPIconf", format=self.RPIFileSort)
        ...
        f = open(self.piDir + "dataVersion", u"w")
        f.write(u"%6.2f"%dataVersion)
        f.close()

        self.writeJson( self.beaconsUUIDtoName, fName=self.piDir + u"beaconsUUIDtoName")

        self.writeJson( self.beaconsIgnoreUUID, fName=self.piDir + u"beaconsIgnoreUUID")

        self.writeJson(self.beaconsUUIDtoIphone,fName=self.piDir + u"beaconsUUIDtoIphone")


####-------------------------------------------------------------------------####
    def saveBeacons(self):
        self.writeJson(self.beacons, fName=self.piDir + u"beacons", format=self.beaconsFileSort)
        ...
        try:
            ...
            if typeId.find(u"rPI") > -1:
                ...
                if pi >= 0:
                    if u"shutDownPinInput"  not in props  or (u"shutDownPinInput"  in props and valuesDict[u"shutDownPinInput"]  != props[u"shutDownPinInput"]):
                        self.setONErPiV(pi,"piUpToDate", [u"updateParams"])
            ...
            if typeId.find(u"INPUTgpio")>-1 or typeId.find(u"INPUTtouch")>-1:
                ...
                for pi0 in range(_GlobalConst_numberOfRPI):
                    ...
                    self.setONErPiV(pi0,"piUpToDate",[u"updateParams"])
            ...
            if typeId.find(u"OUTPUTgpio-") > -1:
                ...
                if pi >= 0:
                    if u"piServerNumber" in props:
                         if pi != int(props[u"piServerNumber"]):
                             ...
                             self.setONErPiV(props[u"piServerNumber"],"piUpToDate",[u"updateParams"])
                    ...
                    if unicode(dev.id) not in self.RPI[unicode(pi)][u"input"][typeId]:
                        ...
                        self.updateNeeded += " fixConfig "
                    self.setONErPiV(pi,"piUpToDate", [u"updateParams"])
                #self.ML.myLog( text = " sensor RPI: "+unicode(self.RPI[unicode(pi)]) )
                ...
                elif ("i2c" in typeId or typeId == "bme680" or typeId == "amg88xx" or typeId == "sgp30" or typeId == "ccs811" or typeId == "as726x" or typeId.find("ina") >-1)  and "i2cAddress" in valuesDict:
                    try:
                ...
                if typeId==u"neopixel-dimmer":
                    ...
                    if pi >= 0:
                        ...
                        if typeId==u"display":
                            ...
                            self.setONErPiV(pi,"piUpToDate", [u"updateAllFiles"]) # this will send images and fonts too
                            valuesDict,error = self.addBracketsPOS(valuesDict,"pos1")
                            if error ==u"":
                                valuesDict,error = self.addBracketsPOS(valuesDict,"pos2")
                                if error ==u"":
                        ...
                        if typeId==u"setTEA5767":
                            dev = indigo.devices[devId]
                            self.setONErPiV(pi,"piUpToDate", [u"updateParams"]) # this will send config only
...
####-------------------------------------------------------------------------####
    def removeONErPiV(self,pi, item, value):
        ...
        if pi in self.RPI:
            for v in value:
                if v in self.RPI[pi][item]:
                    self.RPI[pi][item].remove(v)
        ###self.ML.myLog( text =  u"after"+  unicode(self.RPI[pi][item]))
        return
...
####-------------------------------------------------------------------------####
    def sendConfigCALLBACKaction(self, action1=None, typeId="", devId=0):
        try:
            ...
            return self.execButtonConfig(valuesDict, level="0,", action=[u"updateParams"], Text="send Config Files to pi# ")
        except:
...
####-------------------------------------------------------------------------####
    def buttonConfirmSendOnlyCALLBACK(self, valuesDict=None, typeId="", devId=0):
        return self.execButtonConfig(valuesDict, level="0,", action=[u"updateParams"], Text="send Config Files to pi# ")

####-------------------------------------------------------------------------####
    def buttonConfirmSendRestartPyCALLBACK(self, valuesDict=None, typeId="", devId=0):
        return self.execButtonConfig(valuesDict, level="master,", action=[u"updateParams"], Text="send Config Files and restart master.py  ")

####-------------------------------------------------------------------------####
    def buttonConfirmConfigureCALLBACK(self, valuesDict=None, typeId="", devId=0):
        return self.execButtonConfig(valuesDict, level="reboot,", action=[u"updateParams"], Text="send Config Files and restart rPI")

    def buttonUpgradeCALLBACK(self, valuesDict=None, typeId="", devId=0):
        return self.execButtonConfig(valuesDict, level="0,", action=[u"upgradeOpSys"], Text="upgrade rPi")

####-------------------------------------------------------------------------####
    def buttonResetOutputCALLBACK(self, valuesDict=None, typeId="", devId=0):
        return self.execButtonConfig(valuesDict, level="0,", action=[u"resetOutput"], Text="reset output file  and reboot pi# ")

####-------------------------------------------------------------------------####
    def buttonSendBigFilesCALLBACK(self, valuesDict=None, typeId="", devId=0):
        return self.execButtonConfig(valuesDict, level="master,", action=[u"updateAllFiles"], Text="send ALL Files to pi# ")

####-------------------------------------------------------------------------####
    def buttonShutdownCALLBACK(self, valuesDict=None, typeId="", devId=0):
        self.currentlyBooting = time.time()+self.bootWaitTime
        return self.execButtonConfig(valuesDict, level="0,", action=[u"shutdown"], Text="shut down rPi#  ")

####-------------------------------------------------------------------------####
    def buttonSendAllandRebootCALLBACK(self, valuesDict=None, typeId="", devId=0):
        self.currentlyBooting = time.time()+self.bootWaitTime
        return self.execButtonConfig(valuesDict, level="0,", action=[u"updateAllFiles","reboot"], Text="rPi configure and reboot pi# ")

####-------------------------------------------------------------------------####
    def buttonRebootCALLBACK(self, valuesDict=None, typeId="", devId=0):
        self.currentlyBooting = time.time()+self.bootWaitTime
        return self.execButtonConfig(valuesDict, level="0,", action=[u"reboot"], Text="rPi reboot")
...
####-------------------------------------------------------------------------####
    def buttonGetSystemParametersCALLBACK(self, valuesDict=None, typeId="", devId=0):
        return self.execButtonConfig(valuesDict, level="0,", action=[u"getStats"], Text="get stats from rpi")
...
####-------------------------------------------------------------------------####
    def buttonShutdownHardSocketCALLBACK(self, valuesDict=None ,typeId="", devId=0):
        ...
        if pi == 999:
            for pi in range(_GlobalConst_numberOfRPI):
                out= json.dumps([{u"command":"general","cmdLine":"sync;sleep 2 ;sudo halt &"}])
...
####-------------------------------------------------------------------------####
    def buttonRebootHardSocketCALLBACK(self, valuesDict=None, typeId="", devId=0):
        ...
        if pi == 999:
            for pi in range(_GlobalConst_numberOfRPI):
                out= json.dumps([{u"command":"general","cmdLine":"sync;sleep 2 ;sudo reboot -f &"}])
...
####-------------------------------------------------------------------------####
    def buttonRebootSocketCALLBACK(self, valuesDict=None, typeId="", devId=0):
        ...
        if pi == 999:
            for pi in range(_GlobalConst_numberOfRPI):
                out= json.dumps([{u"command":"general","cmdLine":"sudo reboot &"}])
...
####-------------------------------------------------------------------------####
    def filterSoundFiles(self, valuesDict=None, filter="", typeId="", devId=0):
        list = []
        for fileName in os.listdir(self.piDir+"/soundFiles/"):
...
####-------------------------------------------------------------------------####
    def filterMACs(self, valuesDict=None, typeId="", devId=0):
        ...
        if False:
            try:
                f = open(self.piDir + "rejected/rejectedByPi.json", u"r")
    ...
    def buttonConfirmMACunIgnoreCALLBACK(self, valuesDict=None, typeId="", devId=0):
        ...
        if mac in self.beacons:
            ...
            if mac in self.rejectedByPi:
                ...
                try:
                    f = open(self.piDir + "rejected/rejectedByPi.json", u"w")
...
####-------------------------------------------------------------------------####
    def buttonConfirmMACDeleteALLCALLBACK(self, valuesDict=None, typeId="", devId=0):
        ...
        for pi in range(_GlobalConst_numberOfiBeaconRPI):
            ...
            self.upDateNotSuccessful[pi] = 0
        try:
            os.system(u"rm "+ self.piDir + "rejected/reject-1" )
            os.system(u"cp "+ self.piDir + "rejected/rejects "+ self.piDir + "rejected/reject-1" )
            os.system(u"rm "+ self.piDir + "rejected/rejects*" )
        except: pass
        return valuesDict
...
### not used 
    def buttonConfirmMACnonactiveCALLBACK(self, valuesDict=None, typeId="", devId=0):
        ...
        try:
            ...
            os.system(u"cp "+ self.piDir + "rejected/rejects "+ self.piDir + "rejected/reject-1" )
            os.system(u"rm "+ self.piDir + "rejected/rejects*" )
            self.ML.myLog( text =  u"old rejected/rejects file renamed to"+self.piDir+" rejected/reject-1")
        except: pass
            ...
            if u"command" in vd:    
                ...
                for ii in range(len(cmds)):
                    ...
                    if cType == "0" or cType == "" or \
                        cType not in [u"text",u"textWformat",u"clock",u"dateString",u"analogClock",u"date",u"NOP",u"line",u"point",u"ellipse",u"vBar",u"hBar",u"vBarwBox",u"hBarwBox",u"rectangle",u"triangle",u"hist",u"exec",u"image",u"dot"]:
                        delCMDS.append(ii)
                        continue
                ...
                for ii in range(100):
                    ...
                    if cType == "0":                  continue  
                    if cType == "":                   continue  
                    if cType in [u"text",u"textWformat",u"clock",u"dateString",u"analogClock",u"date",u"NOP",u"line",u"point",u"ellipse",u"vBar",u"hBar",u"vBarwBox",u"hBarwBox",u"rectangle",u"triangle",u"hist",u"exec",u"image",u"dot"]:
                        cmds.append({})
                        nn+=1
    ...
    def setPin(self, valuesDict=None):
        ...
        try:
            ...
            try:
                ...
                self.ML.myLog( text =  u" setPIN bad parameter: OUTPUT device not created: for pi: " + unicode(pi) + "  " + unicode(devId) )
                return
            ...
            if typeId.find(u"OUTPUTgpio") > -1 :
                ...
                try:
                    ...
                    if cmd == "up" or cmd == "down":
                        ...
                        out = cmd + "," + unicode(analogValue)
                    outN = int(output)
                    self.addToStatesUpdateDict(unicode(dev.id),"OUTPUT_%0.2d"%outN, out,dev=dev)
                except  Exception, e:
                    self.ML.myLog( text =  u"setPIN in Line '%s' has error='%s'" % (sys.exc_traceback.tb_lineno, e))
                    outN = 0
                try:
...
####-------------------------------------------------------------------------####
    def buttonConfirmIPnumberCALLBACK(self, valuesDict=None, typeId="", devId=0):
        ...
        if valuesDict[u"oldipNumberPi"] != valuesDict[u"newipNumberPi"]:
            ...
            self.setONErPiV(pi,"piUpToDate",[u"updateParams","reboot"])
            self.rPiRestartCommand[pi]      = "reboot"  ## which part need to restart on rpi
            self.configureWifi(pi)
...
####-------------------------------------------------------------------------####
    def buttonConfirmPiServerConfigCALLBACK(self, valuesDict=None, typeId="", devId=0):
        ...
        #### check pi on/off
            ...
            if p01 ==u"0":
                self.upDateNotSuccessful[pi] == 0
                self.removeONErPiV(pi, u"piUpToDate", [u"updateParams"])
        ...
        #### check userid password ..
            ...
            if pi >= _GlobalConst_numberOfiBeaconRPI:
                        if self.RPI[unicode(pi)][u"piDevId"] == 0: # check if  existing device
                            found =False
                            for dev in indigo.devices.iter(self.pluginId):
                                if dev.deviceTypeId !="rPI-Sensor": continue
...
####-------------------------------------------------------------------------####
    def validatePrefsConfigUi(self, valuesDict):
        ...
        self.logFileActive                  = valuesDict[u"logFilePath"]
        if self.logFileActive =="no":
            self.logFileName =""
            indigo.server.log("logfile handling: regular indigo logfile")
        elif self.logFileActive =="indigo":
        ...
        self.debugLevel         = []
        if valuesDict[u"debugLogic"]:         self.debugLevel.append(u"Logic")
        if valuesDict[u"debugDevMgmt"]:       self.debugLevel.append(u"DevMgmt")
        if valuesDict[u"debugBeaconData"]:    self.debugLevel.append(u"BeaconData")
        if valuesDict[u"debugSensorData"]:    self.debugLevel.append(u"SensorData")
        if valuesDict[u"debugOutputDevice"]:  self.debugLevel.append(u"OutputDevice")
        if valuesDict[u"debugUpdateRPI"]:     self.debugLevel.append(u"UpdateRPI")
        if valuesDict[u"debugOfflineRPI"]:    self.debugLevel.append(u"OfflineRPI")
        ...
        try:               
            if self.debugRPILevel[u"debugRPICALL"]     != int(valuesDict[u"debugRPICALL"]):    self.setALLrPiV(u"piUpToDate", [u"updateParams"])
            self.debugRPILevel[u"debugRPICALL"]         = int(valuesDict[u"debugRPICALL"])
        except:            pass
        try:               
            if self.debugRPILevel[u"debugRPIBEACON"]   != int(valuesDict[u"debugRPIBEACON"]):  self.setALLrPiV(u"piUpToDate", [u"updateParams"])
            self.debugRPILevel[u"debugRPIBEACON"]       = int(valuesDict[u"debugRPIBEACON"])
        except:            pass
        try:               
            if self.debugRPILevel[u"debugRPISENSOR"]   != int(valuesDict[u"debugRPISENSOR"]):  self.setALLrPiV(u"piUpToDate", [u"updateParams"])
            self.debugRPILevel[u"debugRPISENSOR"]       = int(valuesDict[u"debugRPISENSOR"])
        except:            pass
        try:               
            if self.debugRPILevel[u"debugRPIOUTPUT"]   != int(valuesDict[u"debugRPIOUTPUT"]):  self.setALLrPiV(u"piUpToDate", [u"updateParams"])
            self.debugRPILevel[u"debugRPIOUTPUT"]       = int(valuesDict[u"debugRPIOUTPUT"])
        except:            pass
        try:               
            if self.debugRPILevel[u"debugRPIBLE"]      != int(valuesDict[u"debugRPIBLE"]):     self.setALLrPiV(u"piUpToDate", [u"updateParams"])
            self.debugRPILevel[u"debugRPIBLE"]          = int(valuesDict[u"debugRPIBLE"])
        except:            pass
        try:               
            if self.debugRPILevel[u"debugRPImystuff"]  != int(valuesDict[u"debugRPImystuff"]): self.setALLrPiV(u"piUpToDate", [u"updateParams"])
            self.debugRPILevel[u"debugRPImystuff"]      = int(valuesDict[u"debugRPImystuff"])
        except:            pass
        try:        
            if unicode(self.acceptNewiBeacons) != unicode(valuesDict[u"acceptNewiBeacons"]): self.setALLrPiV(u"piUpToDate", [u"updateParams"])
            self.acceptNewiBeacons              = int(valuesDict[u"acceptNewiBeacons"])
        except:            pass

        if unicode(self.acceptJunkBeacons) != unicode(valuesDict[u"acceptJunkBeacons"]): self.setALLrPiV(u"piUpToDate", [u"updateParams"])
        self.acceptJunkBeacons              = valuesDict[u"acceptJunkBeacons"]

        if self.sendFullUUID               != valuesDict[u"sendFullUUID"]: self.setALLrPiV(u"piUpToDate", [u"updateParams"])
        ...
        self.ML.myLogSet(debugLevel = self.debugLevel ,logFileActive=self.logFileActive, logFile = self.logFileName, maxFileSize = 30000000)
...
####-------------------------------------------------------------------------####
    def initConcurrentThread(self):
        ...
        if self.version == u"updateNeeded" :
            self.currentlyBooting = time.time() + 40
            self.ML.myLog( text =  u" ..  new py programs  etc will be send  to rPis")
            for pi in range(_GlobalConst_numberOfRPI) :
                if self.RPI[unicode(pi)][u"ipNumberPi"] != "" :
                    self.setONErPiV(pi,"piUpToDate", [u"updateAllFiles","restartmaster"])
                    self.upDateNotSuccessful[pi] = 0
                    self.sendFilesToPiServerFTP(pi, fileToSend="updateAllFiles.exp")
                    self.sshToRPI(pi, fileToSend=u"restartmaster.exp")
                    self.removeONErPiV(pi,u"piUpToDate", [])
                    self.upDateNotSuccessful[pi] = 0
...
####-------------------------------------------------------------------------####
    def periodCheck(self):
        ...
        try:
            ...
            self.queueList = u"periodCheck"      # block incoming meaasages from processing
            ...
            if not self.findAnyTaskPi(u"piUpToDate"):
               self.updateRejectListsCount =0
                    ...
                    for pi in range(_GlobalConst_numberOfRPI):
                        ...
                        if u"upgradeOpSys" in self.RPI[unicode(pi)][u"piUpToDate"]:
                            self.sshToRPI(pi, fileToSend="upgradeOpSys.exp")
                            self.RPI[unicode(pi)][u"piUpToDate"] = []

                        if u"resetGPIOCount" in self.RPI[unicode(pi)][u"piUpToDate"]:
                            self.sendFilesToPiServerFTP(pi, fileToSend="resetGPIOCount.exp")
                            self.RPI[unicode(pi)][u"piUpToDate"] = []

                        if u"updateAllFiles" in self.RPI[unicode(pi)][u"piUpToDate"]:
                            self.sendFilesToPiServerFTP(pi, fileToSend="updateAllFiles.exp")

                        if u"updateParams" in self.RPI[unicode(pi)][u"piUpToDate"]:
                            self.sendFilesToPiServerFTP(pi, fileToSend="updateParams.exp")
                    ...
                    if self.findTaskPi(u"piUpToDate","getStats"):
                        for pi in range(_GlobalConst_numberOfRPI) :
                            if u"getStats" in self.RPI[unicode(pi)][u"piUpToDate"]:
                                self.sshToRPI(pi,fileToSend="getStats.exp")



                    if self.findTaskPi(u"piUpToDate","shutdown"):
                        for pi in range(_GlobalConst_numberOfRPI) :
                            if u"shutdown" in self.RPI[unicode(pi)][u"piUpToDate"]:
                                self.sshToRPI(pi,fileToSend="shutdown.exp")

                    if self.findTaskPi(u"piUpToDate","reboot"):
                        for pi in range(_GlobalConst_numberOfRPI) :
                            if u"reboot" in self.RPI[unicode(pi)][u"piUpToDate"]  and not  "updateParams" in self.RPI[unicode(pi)][u"piUpToDate"]:
                                reboot=""
                                try: 
                                ...
                                if reboot !="":
                                    self.sshToRPI(pi,fileToSend="rebootF.exp")
                                else:
                                    self.sshToRPI(pi,fileToSend="reboot.exp")

                    if self.findTaskPi(u"piUpToDate","resetOutput"):
                        for pi in range(_GlobalConst_numberOfRPI) :
                            if u"resetOutput" in self.RPI[unicode(pi)][u"piUpToDate"]  and not  "updateParams" in self.RPI[unicode(pi)][u"piUpToDate"]:
                                self.sshToRPI(pi,fileToSend="resetOutput.exp")
    ...
    def BLEconnectCheckPeriod(self, now, force = False):
        try:
            if time.time()< self.currentlyBooting:
                return
            for dev in indigo.devices.iter(self.pluginId +",props.isBLEconnectDevice"):
...
####-------------------------------------------------------------------------####
    def checkSensorStatus(self):
        ...
        try:
            for dev in indigo.devices.iter(self.pluginId +", props.isSensorDevice"):
            ...
            for beacon in self.beacons :
                ...
                if self.beacons[beacon][u"status"] == u"up" :
                    ...
                    if delta < expT and self.beacons[beacon][u"fastDown"] != "0":
                        ...
                        self.beacons[beacon][u"updateFING"] = 1
                        changed = True
                elif self.beacons[beacon][u"status"] == u"expired" and delta < 2 * expT and self.beacons[beacon][u"fastDown"] != u"0":
            ...
            try:
                if now.hour == 0:
                    ...
                    for pi in range(0, _GlobalConst_numberOfRPI) :
                        self.sendFilesToPiServerFTP(pi, fileToSend=u"updateParams.exp")
                    self.updateRejectLists()
            except  Exception, e:
                if len(unicode(e)) > 5 :
                    self.ML.myLog( text =  u"checkHour in Line '%s' has error='%s'" % (sys.exc_traceback.tb_lineno, e))
...
####-------------------------------------------------------------------------####
    def syncSensors(self):
        #self.ML.myLog( text =  u"doing sensor Sync ")
        try:
            for dev in indigo.devices.iter(self.pluginId +", props.isSensorDevice, props.isOutputDevice"):
                ...
                if u"piServerNumber" in props:
                    try:
                        ...
                        continue
                    self.checkDevToPi(pi, devId, dev.name, u"input",  u"in",  sensor, _GlobalConst_allowedSensors)
                    self.checkDevToPi(pi, devId, dev.name, u"output", u"out", sensor, _GlobalConst_allowedOUTPUT)

            for pi in range(_GlobalConst_numberOfRPI):
                self.checkSensortoPi(pi, u"input")
                self.checkSensortoPi(pi, u"output")
                self.mkSensorList(pi)
...
####-------------------------------------------------------------------------####
    def checkDevToPi(self, pi, devId, name, io, io2, sensor, allowedS):
        try:
            if sensor not in allowedS: return
...
####-------------------------------------------------------------------------####
    ...
    def getCurrentState(self,dev,devIds,state,fromMETHOD=""):
        try:
            ...
            if devIds in self.updateStatesDict and state in self.updateStatesDict[devIds]["keys"]:
                ...
                return self.updateStatesDict[devIds]["keys"][state][0] 
            else:  
        ...
        try:
            ...
            for pi1 in range(_GlobalConst_numberOfiBeaconRPI):
                ...
                if signal ==-999:
                    ...
                    elif time.time()- piT2 < 2*expirationTime   and status == "expired": status = "down"
...
####-------------------------------------------------------------------------####
    def setStatusCol(self,dev,key,value,valueUI,whichKeysToDisplay,image,oldStatus,decimalPlaces=1):
        try:
            newStatus = oldStatus
            ...
            if unicode(dev.states[key]) != unicode(value):
                self.addToStatesUpdateDict(unicode(dev.id), key, value,decimalPlaces=decimalPlaces,dev=dev)
                ...
                for i in range(whichKeysToDisplaylength):
                    if whichKeysToDisplayList[i] == key:
                        if currentDisplay[i] != valueUI:
                            ...
                            self.addToStatesUpdateDict(unicode(dev.id),u"status", newStatus,dev=dev)
                            break
            ...
            for NNN in data[u"temp"]:
                ...
                for serialNumber in NNN:
                    ...
                    if dev.states[u"serialNumber"] == "" or  dev.states[u"serialNumber"] == serialNumber: # ==u"" new, ==Serial# already setup
                        ...
                        foundSelf   = False
                        for dev0 in indigo.devices.iter(self.pluginId):
    ...
    def updatePULSE(self,dev,data,whichKeysToDisplay):
        ...
        try:
            ...
            if u"count" in data and str(dev.states[u"count"]) != str(data[u"count"]) :
                ...
                self.addToStatesUpdateDict(str(dev.id),"count",data[u"count"],dev=dev)
                self.addToStatesUpdateDict(str(dev.id),"lastCountSecs",(u"%10.1f"%time.time()).strip())
                self.addToStatesUpdateDict(str(dev.id),"frequency",(u"%10.3f"%freq).strip())
                if   whichKeysToDisplay ==u"count": 
                    self.addToStatesUpdateDict(str(dev.id),"status",data[u"count"])
                elif whichKeysToDisplay ==u"frequency": 
                    self.addToStatesUpdateDict(str(dev.id),"status",(u"%10.3f"%freq).strip()+"[Hz]")
                
            if u"burst" in data and data[u"burst"] !=0 and data[u"burst"] !="":
                    self.addToStatesUpdateDict(str(dev.id),"lastBurstSecs",(u"%10.1f"%time.time()).strip(),dev=dev)
                    if   whichKeysToDisplay ==u"dateOfBurst": 
                        self.addToStatesUpdateDict(str(dev.id),"status",dd,dev=dev)
                
            if u"longEvent" in data and data[u"longEvent"] !=0 and data[u"longEvent"] !="":
                    self.addToStatesUpdateDict(str(dev.id),"lastLongEventSecs",(u"%10.1f"%time.time()).strip(),dev=dev)
                    if   whichKeysToDisplay ==u"dateOflongEvent": 
                        self.addToStatesUpdateDict(str(dev.id),"status",dd)
                
            if u"continuous" in data and data[u"continuous"] !="":
                    if data[u"continuous"] > 0: 
                        self.addToStatesUpdateDict(str(dev.id),"lastContinuousEventSecs",(u"%10.1f"%time.time()).strip(),dev=dev)
                        if   whichKeysToDisplay ==u"continousEventON": 
                            self.addToStatesUpdateDict(str(dev.id),"status","ON")
                        elif   whichKeysToDisplay ==u"continousEventDate": 
                            self.addToStatesUpdateDict(str(dev.id),"status",dd+"-ON")
                    else: 
                        self.addToStatesUpdateDict(str(dev.id),"lastContinuousEventSecsStop",(u"%10.1f"%time.time()).strip(),dev=dev)
                        if   whichKeysToDisplay ==u"continousEventON": 
                            self.addToStatesUpdateDict(str(dev.id),"status","off")
                        elif   whichKeysToDisplay ==u"continousEventDate": 
                            self.addToStatesUpdateDict(str(dev.id),"status",dd+"-off")
...
####-------------------------------------------------------------------------####
    def updateina219(self,dev,data,upState):
        try:
            input = u"Current"
            props = dev.pluginProps
            if unicode(data[input]).find(u"bad") >-1:
            ...
            if input in data:
                    if data[input] !="":
                        ...
                        if  upState == input:
                            self.addToStatesUpdateDict(unicode(dev.id),u"status", ssUI+unit,dev=dev)
                        self.addToStatesUpdateDict(unicode(dev.id),input,ss,dev=dev)
                    
            input = u"ShuntVoltage"
            if input in data:
                    if data[input] !="":
                        ss, ssUI, unit = self.addmultOffsetUnit(data[input], dev.pluginProps)
                        if  upState == input:
            ...
            if self.ML.decideMyLog(u"SensorData"): self.ML.myLog( text =  u"updateina219 in Line '%s' has error='%s'" % (sys.exc_traceback.tb_lineno, e))



####-------------------------------------------------------------------------####
    def updateina3221(self,dev,data,upState):
        try:
            for ii in range(1,4):
                props = dev.pluginProps
                input = u"Current"+str(ii)
                if unicode(data[input]).find(u"bad") >-1:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status", u"no sensor data - disconnected?",dev=dev)
                        self.setIcon(dev,props,"SensorOff-SensorOn",0)
                        return
...
####-------------------------------------------------------------------------####
    def updateADC121(self,dev,data,upState):
        try:
            input = u"adc"
            props = dev.pluginProps
            type  = props[u"type"]
            ...
            if input in data:
                    if data[input] !="":
                        ...
                        self.addToStatesUpdateDict(unicode(dev.id),u"status", ssUI+unit,dev=dev)
                        self.addToStatesUpdateDict(unicode(dev.id),"value",ss)
                        self.addToStatesUpdateDict(unicode(dev.id),"adc",ADC)
        except  Exception, e:
        ...
        try:
            ...
            if input in data:
                ...
                if units == "cm":
                   ...
                   dist0 = (u"%8.2f"%(dist)).replace(u" ","")
                if u"distance" in whichKeysToDisplay:
                    self.addToStatesUpdateDict(str(dev.id),u"status", dist0 + ud,dev=dev)
                self.addToStatesUpdateDict(unicode(dev.id),input, dist0,dev=dev)
                ...
                if u"speed" in data:
                    try: 
                        ...
                        if units == 0.01:
                           ud = " [cm/s]"
                           speed = (u"%8.1f"%(speed)).replace(u" ","")
                        elif units == 1.0:
                           ud = " [m/s]"
                           speed = (u"%8.2f"%(speed)).replace(u" ","")
                        elif units == 0.0254:
                           ud = ' [i/s]'
                           speed = (u"%7.1f"%(speed)).replace(u" ","")
                        elif units == 0.348:
                           ud = " [f/s]"
                           speed = (u"%8.2f"%(speed)).replace(u" ","")
                        elif units == 0.9144:
                           ...
                           ud = " [mph]"
                           speed = (u"%8.2f"%(speed)).replace(u" ","")
                        self.addToStatesUpdateDict(unicode(dev.id),"speed", speed,dev=dev)
                        if u"speed" in whichKeysToDisplay:
                            self.addToStatesUpdateDict(str(dev.id),u"status", speed + ud)
                    except:
                        pass
    ...
    def updateLight(self, dev, data, upState,theType=""):
        try:
            if u"illuminance" in data or u"  lux" in data or "UV" in data or "UVA" in data or "UVB" in data or "IR" in data or "ambient" in data or "white"  or "visible" in data:
                ...
                if u"lux" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"illuminance", float(data[u"lux"]),decimalPlaces=2,dev=dev)
                    self.fillMinMaxSensors(dev,"illuminance",data[u"lux"],decimalPlaces=2)
                if u"illuminance" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"illuminance", float(data[u"illuminance"]),decimalPlaces=2,dev=dev)
                    self.fillMinMaxSensors(dev,"illuminance",data[u"illuminance"],decimalPlaces=2)
                if u"IR" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"IR",  float(data[u"IR"]),decimalPlaces=2,dev=dev)
                    self.fillMinMaxSensors(dev,"IR",data[u"IR"],decimalPlaces=2)
                if u"UVA" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"UVA", float(data[u"UVA"]),decimalPlaces=2,dev=dev)
                if u"UVB" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"UVB", float(data[u"UVB"]),decimalPlaces=2,dev=dev)
                if u"UV" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"UV", float(data[u"UV"]),decimalPlaces=2,dev=dev)
                if u"ambient" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"ambient",float(data[u"ambient"]),decimalPlaces=2,dev=dev)
                    self.fillMinMaxSensors(dev,"ambient",data[u"ambient"],decimalPlaces=2)
                if u"white" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"white",float(data[u"white"]),decimalPlaces=2,dev=dev)
                    self.fillMinMaxSensors(dev,"white",data[u"white"],decimalPlaces=2)
                if u"visible" in data:
                    self.addToStatesUpdateDict(unicode(dev.id),u"visible",float(data[u"visible"]),decimalPlaces=2,dev=dev)
                    self.fillMinMaxSensors(dev,"visible",data[u"visible"],decimalPlaces=2)

                if   upState == "illuminance" and "lux" in data:
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"lux"]))) ).replace(u" ", u"")+unit,dev=dev)
                    else:    
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"lux"])).replace(u" ", u"")+unit,dev=dev)
                elif   upState == "illuminance" and "illuminance" in data:
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"illuminance"]))) ).replace(u" ", u"")+unit,dev=dev)
                    else:    
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"illuminance"]) ).replace(u" ", u"")+unit,dev=dev)
                elif upState == "IR":
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"IR"])))).replace(u" ", u"")+unit,dev=dev)
                    else:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"IR"])+"[IR]").replace(u" ", u"")+unit,dev=dev)
                elif upState == "ambient":
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"ambient"])))).replace(u" ", u"")+unit,dev=dev)
                    else:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"ambient"])).replace(u" ", u"")+unit,dev=dev)
                elif upState == "white":
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"white"])))).replace(u" ", u"")+unit,dev=dev)
                    else:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"white"])).replace(u" ", u"")+unit,dev=dev)
                elif upState == "UV":
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"UV"])))).replace(u" ", u"")+unit,dev=dev)
                    else:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"UV"])).replace(u" ", u"")+unit,dev=dev)
                elif upState == "UVA":
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"UVA"]))) ).replace(u" ", u"")+unit,dev=dev)
                    else:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"UVA"])).replace(u" ", u"")+unit,dev=dev)
                elif upState == "UVB":
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"UVB"]))) ).replace(u" ", u"")+unit,dev=dev)
                    else:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"UVB"])).replace(u" ", u"")+unit,dev=dev)
                elif upState == "visible":
                    if logScale ==u"1":
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % math.log10(max(0.1,float(data[u"visible"]))) ).replace(u" ", u"")+unit,dev=dev)
                    else:
                        self.addToStatesUpdateDict(unicode(dev.id),u"status",(formatN % float(data[u"visible"])).replace(u" ", u"")+unit,dev=dev)
...
####-------------------------------------------------------------------------####
    def updateRGB(self, dev, data, upState, theType="",dispType ="S"):
        try:
            props = dev.pluginProps
            if u"unit" in props: unit = props[u"unit"]
            else:                unit = ""
            ...
            if changed > 0:
                    ...
                    if u"illuminance" in dev.states and u"illuminance" not in data:
                        ...
                        il1 = (u"%6.1f" % il).replace(u" ", u"")
                        ilUI = (u"%6.1f" % il + "[Lux]").replace(u" ", u"")
                        self.addToStatesUpdateDict(unicode(dev.id),"illuminance",il1 ,dev=dev)
                        if upState == "illuminance":
                            self.addToStatesUpdateDict(unicode(dev.id),u"status", ilUI,dev=dev)
                    if u"kelvin" in dev.states:
                        k = int(self.calcKelvin(data))
                        self.addToStatesUpdateDict(unicode(dev.id),"kelvin", unicode(k),dev=dev)
                        if upState == "kelvin":
                            self.addToStatesUpdateDict(unicode(dev.id),u"status", unicode(k) + u"[ºK]",dev=dev)
...
####-------------------------------------------------------------------------####
    def updateRGB2(self, dev, color, data, upState,unit, dispType="S"):
        ...
        try:
            ...
            if dispType !="S":
                ...
                self.addToStatesUpdateDict(unicode(dev.id),color, float(data[color]),decimalPlaces=dispType ,dev=dev)
                if upState == color:
                    self.addToStatesUpdateDict(unicode(dev.id),u"status", color+" "+unicode(data[color])+unit,decimalPlaces=dispType ,dev=dev)
                return 1

            if dev.states[color] != unicode(data[color]):
                self.addToStatesUpdateDict(unicode(dev.id),color, unicode(data[color]),dev=dev)
                if upState == color:
                    self.addToStatesUpdateDict(unicode(dev.id),u"status", "color "+unicode(data[color])+unit,dev=dev)
                return 1
            return 0
        except  Exception, e:
            if len(unicode(e)) > 5 :
    ...
    def convTemp(self, temp):
        try:
            ...
            if self.tempUnits == u"Fahrenheit":
                ...
                suff = u"ºC"
            tempU = (u"%4.1f" % temp).strip()
            return temp, tempU + suff,1
        except:
            return -99, u"",1
    ...
    def updateBeaconStates(self, fromPi, piNReceived, ipAddress, piMACSend, nOfSecs, msgs):
        ...
        try:
            ...
            if not foundPI:
                ...
                self.RPI[unicode(fromPi)][u"piNumberReceived"]   = piNReceived
                self.RPI[unicode(fromPi)][u"piMAC"]              = piMACSend
                self.setONErPiV(fromPi,"piUpToDate", [u"updateParams","reboot"])
            ...
            for msg in msgs:
                ...
                if (len(uuid) > 11 and uuid[:12] in self.beaconsIgnoreUUID) or (mac in self.beacons and self.beacons[mac][u"ignore"] >0 ):
                    rj = open(self.piDir + "rejected/rejects", u"a")
...
####-------------------------------------------------------------------------####
    def makeBeacons_parameterFile(self):
        ...
        f = open(self.piDir + "all/beacon_parameters", u"w")
        f.write(json.dumps(out))
        f.close()

        try:
            f = open(self.piDir + "all/touchFile", u"w")
...
####-------------------------------------------------------------------------####
    def makeInterfacesFile(self,piS):
        try:
            ...
            f = open(self.piDir + "interfaceFiles/interfaces." + piS, u"w")
            f.write(u"auto lo\n")
...
####-------------------------------------------------------------------------####
    def makeSupplicantFile(self,piS):
        try:
            if self.RPI[piS][u"piOnOff"] == "0": return
            f = open(self.piDir + "interfaceFiles/wpa_supplicant.conf." + piS, u"w")
...
####-------------------------------------------------------------------------####
    def makeParametersFile(self, piS,retFile=False):
        try:
                ...
                out[u"output"]={}            
                for devOut in indigo.devices.iter(self.pluginId):
                ...
                out = self.writeJson(out, fName = self.piDir + u"interfaceFiles/parameters." + piS , format=self.parametersFileSort )
...
####-------------------------------------------------------------------------####
    def sendFilesToPiServerFTP(self, pi, fileToSend=u"updateParams.exp"):
        try:
            ...
            cmd0+=  self.RPI[unicode(pi)][u"userIdPi"] + " " + self.RPI[unicode(pi)][u"passwordPi"]+" "
            cmd0+=  self.RPI[unicode(pi)][u"ipNumberPiSendTo"] + " "
            cmd0+=  unicode(pi) + " " + self.piDir + " '" + self.pathToPlugin + "pi'" + " "+self.expectTimeout
...
####-------------------------------------------------------------------------####
    def updatepiUpToDate(self, pi,ret):

        if self.ML.decideMyLog(u"UpdateRPI"): self.ML.myLog( text =  u"setup Pi ok: \n" + ret[0].replace(u"\n\n", u"\n")+u"\n....\n"+ ret[0][-300:].replace(u"\n\n", u"\n"))
        if u"resetGPIOCount" in self.RPI[unicode(pi)][u"piUpToDate"]:
            self.removeONErPiV(pi, u"piUpToDate", [u"resetGPIOCount"])
            os.remove(self.piDir + u"interfaceFiles/resetGPIOCount." + unicode(pi))
        elif u"updateAllFiles" in self.RPI[unicode(pi)][u"piUpToDate"]:
            self.removeONErPiV(pi, u"piUpToDate", [u"updateAllFiles"])
        elif u"updateParams" in self.RPI[unicode(pi)][u"piUpToDate"]:
            self.removeONErPiV(pi, u"piUpToDate", [u"updateParams"])
        else:
            self.RPI[unicode(pi)][u"piUpToDate"] = []
...
####-------------------------------------------------------------------------####
    def sshToRPI(self, pi,fileToSend=""):
        try:
            ...
            if fileToSend in [u"shutdown.exp",u"reboot.exp",u"rebootF.exp",u"resetOutput.exp",u"shutdown.exp"]:
                batch =" &"
            else: 
                batch =" "
            cmd = "/usr/bin/expect '" + self.pathToPlugin + fileToSend+"' " + " " + self.RPI[unicode(pi)][u"userIdPi"] + " " + self.RPI[unicode(pi)][u"passwordPi"] + " " + self.RPI[unicode(pi)][u"ipNumberPiSendTo"]+ " "+self.expectTimeout+ " "+batch
    ...
    def configureWifi(self, pi):
        ...
        try:
            ...
            if self.testPing(self.RPI[unicode(pi)][u"ipNumberPiSendTo"]) != 0:
                ...
                return 1
            cmd = "/usr/bin/expect '" + self.pathToPlugin + u"wifi.exp' " + u" " + self.RPI[unicode(pi)][u"userIdPi"] + u" " + self.RPI[unicode(pi)][u"passwordPi"]+ u" "+self.expectTimeout
...
####-------------------------------------------------------------------------####
    def testPing(self, ipN):
        try:
            ret = os.system(u"/sbin/ping  -c 4 -W 1 -o " + ipN) # send max 4 packets, wait 0.5 secs between each and if one gets back stop
            if self.ML.decideMyLog(u"UpdateRPI"): self.ML.myLog( text =  u" sbin/ping  -c 4 -W 1 -o " + ipN+" return-code: " + unicode(ret))
            if int(ret) ==0:  return 0
            self.sleep(1)
            ret = os.system(u"/sbin/ping  -c 4 -W 2 -o " + ipN)
            if self.ML.decideMyLog(u"UpdateRPI"): self.ML.myLog( text =  u" sbin/ping  -c 4 -W 2 -o " + ipN+" ret-code: " + unicode(ret))
            if int(ret) ==0:  return 0
            return 1
        except  Exception, e:
...
####-------------------------------------------------------------------------####
    def printConfig(self):
        ...
        self.ML.myLog( text =  u"data path used                " + unicode(self.piDir),                        mType= u"pi configuration")
...
####-------------------------------------------------------------------------####
    def updateRejectLists(self):
        cmd = self.pythonPath + u" '" + self.pathToPlugin + u"updateRejects.py' '" + self.piDir + u"' & "
        ...
        try:
            try:
                ...
                if devId not in self.updateStatesDict: 
                    self.updateStatesDict[devId]={"keys":{}}
                if key in self.updateStatesDict[devId]["keys"]:
        ...
        try:
            ...
            for devId in local:
                ...
                if len(local) > 0:
                    dev =indigo.devices[int(devId)]
                    nKeys =0
                    for key in local[devId]["keys"]:
                        value = local[devId]["keys"][key][0]
                        if key not in dev.states and key != "lastSensorChange":
                            ...
                            upd = False
                            if local[devId]["keys"][key][1] != "": # decimal places present?
                                try: 
                                    if round(value,local[devId]["keys"][key][1]) !=  round(dev.states[key],local[devId]["keys"][key][1]):
                                        upd=True
                                except: 
                                        upd=True
                            else: 
                            ...
                            if upd:
                                ...
                                changedOnly[devId][key] = local[devId]["keys"][key]
                                if "lastSensorChange" in dev.states and "lastSensorChange" not in changedOnly[devId]:
                                    nKeys +=1
                                    changedOnly[devId]["lastSensorChange"] = [dateString,""]

                    #self.ML.myLog( text =  u"changedOnly "+unicode(changedOnly))
                    if devId in changedOnly and len(changedOnly[devId]) >0:
                        ...
                        for key in changedOnly[devId]:
                            ...
                            if changedOnly[devId][key][1] == "" and key in dev.states:
                                if key != "lastSensorChange" or ( key == "lastSensorChange" and nKeys >0): 
                                    chList.append({u"key":key,"value":changedOnly[devId][key][0]})
                            else:
                                chList.append({u"key":key,"value":changedOnly[devId][key][0], u"decimalPlaces":changedOnly[devId][key][1]})
                            if dev.deviceTypeId ==u"beacon": 
                                if key ==u"status": 
                                    trigStatus           = dev.name
                                    devnamechangedStat   = dev.name+ u"  "+key+ u"  old="+str(dev.states[key])+ u"   new="+str(changedOnly[devId][key][0])
                                if key ==u"closestRPI": 
                                    trigRPIchanged       = dev.name
                                    devnamechangedRPI    = dev.name+ u"  "+key+ u"  old="+str(dev.states[key])+ u"   new="+str(changedOnly[devId][key][0]) 
