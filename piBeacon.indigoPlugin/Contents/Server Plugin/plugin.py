	...
	def setVariables(self):
		try:
			...
			for ii in range(_GlobalConst_numberOfRPI):
				...
				self.rpiQueues["lastActive"][iiU]	= -1
				self.RPIBusy[iiU]					= 0
	...
	def readConfig(self):  ## only once at startup
		try:
			...
			for beacon in self.beacons:
				...
				for pi in range(_GlobalConst_numberOfiBeaconRPI):
					try:
						try:    d =  float(dev.states["Pi_"+unicode(pi)+"_Distance"])
						except: d = 99999.
						...
						self.beacons[beacon][u"receivedSignals"][pi]["rssi"] = float(dev.states[u"Pi_"+unicode(pi)+"_Signal"])
				...
				for pi in range(_GlobalConst_numberOfRPI):
					...
					try:
						...
						if piDevId >0:
							...
							if u"ipNumberPi" not in props or (self.isValidIP(self.RPI[unicode(pi)][u"ipNumberPi"]) and self.RPI[unicode(pi)][u"ipNumberPi"] != props[u"ipNumberPi"]):
								upd=True
						...
						if unicode(e).find(u"timeout waiting") > -1:
							...
							return
						self.sleep(0.2)
						if self.RPI[unicode(pi)][u"piDevId"] !=0:
							try:
								self.indiLOG.log(40,"in Line {} has error={}".format(sys.exc_traceback.tb_lineno, e))
					...
					if self.RPI[unicode(pi)][u"piOnOff"] != "0":
						if not self.isValidIP(self.RPI[unicode(pi)][u"ipNumberPi"]):
							self.RPI[unicode(pi)][u"piOnOff"] = "0"
							anyChange = True
							continue
		except Exception, e:
			...
			if "all" in checkOnly:
				...
				for dev in indigo.devices.iter("props.isCARDevice,props.isBeaconDevice,props.isRPIDevice,props.isRPISensorDevice"):
					...
					if dev.deviceTypeId.find(u"rPI") >-1: 
						...
						if u"ipNumberPi" in props and self.isValidIP(self.RPI[unicode(pi)][u"ipNumberPi"]) and self.RPI[unicode(pi)][u"ipNumberPi"] != props[u"ipNumberPi"]:
							self.indiLOG.log(20, "{} fixing ipNumber in RPI device props to {}".format(dev.name, self.RPI[unicode(pi)][u"ipNumberPi"]))
							dev.description = "Pi-{}-{}".format(pi,self.RPI[unicode(pi)][u"ipNumberPi"])
							dev.replaceOnServer()
							props[u"ipNumberPi"] = self.RPI[unicode(pi)][u"ipNumberPi"]  
							dev.replacePluginPropsOnServer(props)
							anyChange = True

						if u"ipNumberPi" in props and self.isValidIP(props[u"ipNumberPi"]) and self.RPI[unicode(pi)][u"ipNumberPi"] != props[u"ipNumberPi"]:
							...
							if self.beacons[beacon][u"indigoId"] != 0 :# and self.beacons[beacon][u"ignore"] ==0:
								try:
									...
									if unicode(e).find(u"timeout waiting") > -1:
										...
										self.beacons[beacon][u"indigoId"] = 0
										anyChange = True
										#self.indiLOG.log(20,	u"fixConfig anychange: D-2  beacon, pi, devid "+ unicode(beacon) +"  "+ unicode(pi) +"  "+ unicode(devId) )
										continue
									else:
										self.indiLOG.log(40,"in Line {} has error={}".format(sys.exc_traceback.tb_lineno, e))
			...
			if typeId == "rPI":
				try:
					...
					if new:
						...
						for pi in range(_GlobalConst_numberOfiBeaconRPI): 
							if self.RPI[unicode(pi)][u"piMAC"] == newMAC:
								self.indiLOG.log(20, u"adding new RPI another RPI(#{}) has already that this MAC number:{}".format(pi, newMAC ))
								valuesDict, errorDict  = self.setErrorCode(valuesDict,errorDict,  "bad beacon#, already exist as RPI")
								return ( False, valuesDict, errorDict )
				...
				try:
					...
					for pi in range(_GlobalConst_numberOfiBeaconRPI):
						if valuesDict[u"rPiEnable"+unicode(pi)]:
							...
							if typeId in self.RPI[unicode(pi)][u"input"] and  unicode(devId) in self.RPI[unicode(pi)][u"input"][typeId]:
						...
						if typeId in self.RPI[unicode(pi)][u"input"] and self.RPI[unicode(pi)][u"input"][typeId] == {}:
							del self.RPI[unicode(pi)][u"input"][typeId]
						if typeId not in self.RPI[unicode(pi)][u"input"] and typeId in self.RPI[unicode(pi)][u"sensorList"]:
	...
	def printDeviceDictCALLBACK(self, valuesDict=None, typeId=""):
		...
		if u"piServerNumber" in props:
			if props[u"piServerNumber"]!="":
				...
				self.myLog( text = u"\n"+self.writeJson(self.RPI[unicode(pi)], fmtOn=True ),mType="RPI info:")
		else:
			for pi in range(_GlobalConst_numberOfiBeaconRPI):
...
####-------------------------------------------------------------------------####
	def filterAllpiSimple(self, filter="", valuesDict=None, typeId="", devId=""):
		...
		for pi in range(_GlobalConst_numberOfRPI):
			...
			xList.append((piS,"#"+piS+"-"+self.RPI[piS][u"ipNumberPi"]+name))
		return xList
	...
	def filterPiO(self, valuesDict=None, filter="self", typeId="", devId="x"):
			...
			for pi in range(_GlobalConst_numberOfiBeaconRPI):
				...
				if self.RPI[piS][u"piOnOff"] != u"0":
					...
					xList.append([piS,u"#"+piS+"-"+self.RPI[piS][u"ipNumberPi"]+name])
	...
	def filterPiOUT(self, filter="", valuesDict=None, typeId="", devId=0):
		...
		for pi in range(_GlobalConst_numberOfRPI):
			...
			if typeId in self.RPI[unicode(pi)][u"output"] and unicode(devId) in self.RPI[unicode(pi)][u"output"][typeId]:
				...
				try:
					xList.append((unicode(pi), u"Pi-" + unicode(pi) + "-" + self.RPI[unicode(pi)][u"ipNumberPi"] + ";  Name =" + indigo.devices[devIDpi].name))
				except Exception, e:
					self.indiLOG.log(40,"in Line {} has error={}".format(sys.exc_traceback.tb_lineno, e))
					self.indiLOG.log(40, u" devid " + unicode(devIDpi))
	...
	def delRPI(self, pi="", dev="none"):
		try:
			...
			if pi !="":
				...
				self.indiLOG.log(30,"=== delRPI:  deleting pi:{}  devID:{} ".format(pi, devID) )
			...
			if dev !="none":
				devID = dev.id
				self.indiLOG.log(30,"=== delRPI:  deleting dev:{} ".format(dev.name) )
...
####-------------------------------------------------------------------------####
	def initConcurrentThread(self):
		...
		while True:
			...
			self.indiLOG.log(20, u"userid or password not configured in config and security level  digest or basic")
			self.sleep(20)
	...
	def checkForUpdates(self,now):
		...
		try:
			...
			if self.updateNeeded.find(u"enable") > -1 or self.updateNeeded.find(u"disable") > -1: 
				...
				#self.syncSensors()
				self.setupFilesForPi(calledFrom="checkForUpdates enable")
				try:
					pi = self.updateNeeded.split(u"-")[1]
					self.setONErPiV(pi,"piUpToDate", [u"updateParamsFTP"])
					if self.decideMyLog(u"BeaconData"): self.indiLOG.log(10, u"sending update to pi#{}".format(pi))
				except: pass
			...
			if self.selectBeaconsLogTimer !={}: 
				for sMAC in self.selectBeaconsLogTimer:
					if piMAC.find(sMAC[:self.selectBeaconsLogTimer[sMAC]]) ==0:
						self.indiLOG.log(20, u"sel.beacon logging: RPI msg	:"+piMAC+"; "+(" ").ljust(36)  + " pi#="+unicode(pi) )
	...
	def setRPIonline(self,pi,new="up"):
		try:
			...
			try: dev = indigo.devices[self.RPI[unicode(pi)][u"piDevId"]]
				...
				self.indiLOG.log(20,u"setRPIonline looks like device is deleted.. setting pi:{}  devId:{} to delete".format(pi, devID))
				self.delRPI(pi=pi)
				return 
			...
			if new==u"reboot":
				...
				if dev.states[u"online"] != "reboot":
					self.indiLOG.log(20, u"setting status of pi# "+unicode(pi)+"   to reboot for "+unicode(int(self.bootWaitTime))+" seconds or until new message arrives")
					dev.updateStateImageOnServer(indigo.kStateImageSel.SensorOff)
...
####-------------------------------------------------------------------------####
	def updaterainSensorRG11(self, dev, data, whichKeysToDisplay):
		try:
			...
			if "format" in props and len(props[u"format"])<2: form = "%."+unicode(self.rainDigits+1)+"f["+self.rainUnits+"]"
			...
			for msg in msgs:
				...
				if self.selectBeaconsLogTimer !={}: 
					for sMAC in self.selectBeaconsLogTimer:
						if mac.find(sMAC[:self.selectBeaconsLogTimer[sMAC]]) ==0:
							self.indiLOG.log(20, u"sel.beacon logging: newMSG	  -1- :"+mac+"; "+(" ").ljust(36)	 + " pi#="+unicode(fromPi) +"; #Msgs="+unicode(lCount).ljust(2)   +";  pkLen="+unicode(pkLen).ljust(3)						   +"     rssi="+unicode(rssi).rjust(6)	 + "                      txPow="+unicode(txPower).rjust(6)+" uuid="+ uuid.ljust(44))
				...
				if rssi < self.acceptNewiBeacons and name ==u"" and self.beacons[mac][u"ignore"] >= 0: 
					if self.selectBeaconsLogTimer !={}: 
						for sMAC in self.selectBeaconsLogTimer:
							if mac.find(sMAC[:self.selectBeaconsLogTimer[sMAC]]) ==0:
								self.indiLOG.log(20, u"sel.beacon logging: newMSG rej rssi :"+mac+"; "+("name= empty").ljust(30)  + " pi#="+unicode(fromPi) +";  #Msgs="+unicode(lCount).ljust(2)   +";  pkLen="+unicode(pkLen).ljust(3)+"                     + rssi="+unicode(rssi).rjust(6)     + "                      txPow="+unicode(txPower).rjust(6)+" uuid="+ uuid.ljust(44))
				...
				if self.newBeaconsLogTimer >0:
						try:
							...
							if created + self.newBeaconsLogTimer > 2*time.time():
								self.indiLOG.log(20, u"new.beacon logging: newMSG	 -2- :"+mac+";  "+name.ljust(36)+ " pi#="+unicode(fromPi) +";  #Msgs="+unicode(lCount).ljust(2)	  +";  pkLen="+unicode(pkLen).ljust(3)                  + "  rssi="+unicode(rssi).rjust(6)      +"                      txPow="+unicode(txPower).rjust(6)+" cr="+dev.states[u"created"]+" uuid="+ uuid.ljust(44))
				...
				if self.selectBeaconsLogTimer !={}: 
					for sMAC in self.selectBeaconsLogTimer:
						if mac.find(sMAC[:self.selectBeaconsLogTimer[sMAC]]) ==0:
							self.indiLOG.log(20, u"sel.beacon logging: newMSG     -3- :"+mac+"; "+name.ljust(36)       + " pi#="+unicode(fromPi) +";  #Msgs="+unicode(lCount).ljust(2)     +";  pkLen="+unicode(pkLen).ljust(3)                         +"       rssi="+unicode(rssi).rjust(6)   + "                        txPow="+unicode(txPower).rjust(6)+" uuid="+ uuid.ljust(44))

				if logTRUEfromChangeOFRPI:
					self.indiLOG.log(20, u"ChangeOfRPI.beacon logging     :"+mac+"  "+name.ljust(36)       + " pi#="+unicode(closestRPI)+" oldpi=" + unicode(oldRPI)+";   #Msgs="+unicode(lCount).ljust(2)    +";   pkLen="+unicode(pkLen) + "        rssi="+unicode(rssi).rjust(6)        + "                         txPow="+unicode(txPower).rjust(6))
		  
				if logTRUEfromSignal:
					if abs(deltaSignalLOG)	 > 500 and rssi > -200:
						self.indiLOG.log(20, u"ChangeOfSignal.beacon logging:        "+mac+";  "+name.ljust(36)+ " pi#="+unicode(fromPi)     +";  #Msgs="+unicode(lCount).ljust(2)     +";  pkLen="+unicode(pkLen).ljust(3)                         +"       rssi="+unicode(rssi).rjust(6)    +" off --> ON            txPow="+unicode(txPower).rjust(6))
					elif abs(deltaSignalLOG) > 500 and rssi < -200:
						self.indiLOG.log(20, u"ChangeOfSignal.beacon logging:        "+mac+";  "+name.ljust(36)+ " pi#="+unicode(fromPi)     +";  #Msgs="+unicode(lCount).ljust(2)     +";  pkLen="+unicode(pkLen).ljust(3)                         +"       rssi="+unicode(rssi).rjust(6)    +" ON  --> off            txPow="+unicode(txPower).rjust(6))
					else:
						self.indiLOG.log(20, u"ChangeOfSignal.beacon logging:        "+mac+";  "+name.ljust(36)+ " pi#="+unicode(fromPi)     +";  #Msgs="+unicode(lCount).ljust(2)     +";  pkLen="+unicode(pkLen).ljust(3)                         +"       rssi="+unicode(rssi).rjust(6)    +" new-old_Sig.= "+ unicode(deltaSignalLOG).rjust(5)+ "     txPow="+unicode(txPower).rjust(6))
		...
		if state =="start":
			...
			for pi in range(_GlobalConst_numberOfRPI):
				...
				if piSelect == "all" or piU == piSelect:
					self.startOneUpdateRPIqueues(piU)
			...
			if (piSelect == "all" and time.time() - self.laststartUpdateRPIqueues > 70) or piSelect != "all":
				...
				for pi in range(_GlobalConst_numberOfRPI):
					piU= unicode(pi)
					if piSelect == "all" or piU == piSelect:
						if time.time() - self.rpiQueues["lastActive"][piU] > 100:
							self.stopUpdateRPIqueues(piSelect=piU)
						if  time.time() - self.rpiQueues["lastActive"][piU] > 100 or self.rpiQueues["state"][piU]	!= "running":
							self.startOneUpdateRPIqueues(piU)
		return 

####-------------------------------------------------------------------------####
	def startOneUpdateRPIqueues(self, piSelect):
...
####-------------------------------------------------------------------------####
	def resetUpdateQueue(self, pi):
		self.rpiQueues["reset"][unicode(pi)] = True
		return
####-------------------------------------------------------------------------####
...
####-------------------------------------------------------------------------####
	def rpiUpdateThread(self,thisPi):
		...
		try:
			self.rpiQueues["state"][piUIN] = "running"
			if self.decideMyLog(u"UpdateRPI"): self.indiLOG.log(10, u"rpiUpdateThread starting  for pi# {}".format(piUIN) )
			while self.rpiQueues["state"][piUIN] == "running":
				self.rpiQueues["lastActive"][piUIN]  = time.time()
				time.sleep(1)
				addBack =[]
				while not self.rpiQueues["data"][piUIN].empty():
					next 	= self.rpiQueues["data"][piUIN].get()
					if self.decideMyLog(u"UpdateRPI"): self.indiLOG.log(20, u"rpiUpdateThread executing  {}".format(next) )
					if piUIN != unicode(next["pi"]): 			
						if self.decideMyLog(u"UpdateRPI"): self.indiLOG.log(20, u"rpiUpdateThread skipping; pi numbers wrong  {} vs {} ".format(piUIN, next["pi"]) )
						continue
					if self.RPI[piUIN][u"piOnOff"] == "0": 		
						if self.decideMyLog(u"UpdateRPI"): self.indiLOG.log(20, u"rpiUpdateThread skipping;  Pi#: {} is OFF".format(piUIN) )
						continue
					if self.RPI[piUIN][u"ipNumberPi"] == "": 	
						if self.decideMyLog(u"UpdateRPI"): self.indiLOG.log(20, u"rpiUpdateThread skipping pi#:{}  ip number blank".format(piUIN)  )
						continue
					if piUIN in self.rpiQueues["reset"] and self.rpiQueues["reset"][piUIN]: 
						if self.decideMyLog(u"UpdateRPI"): self.indiLOG.log(20, u"rpiUpdateThread resetting queue data for pi#:{}".format(piUIN) )
						continue
					try:
						id = int(self.RPI[piUIN][u"piDevId"])
						if id != 0 and not indigo.devices[id].enabled: 
							self.rpiQueues["reset"][piUIN] = True
					...
					if next["exeTime"] > time.time():
						...
						continue
					self.RPIBusy[piUIN] = time.time()

					if next["type"] =="ftp":
						retCode, text = self.execSendFilesToPiFTP(piUIN, fileToSend=next["fileToSend"], endAction= next["endAction"])
					else:
						retCode, text = self.execSshToRPI(piUIN, fileToSend=next["fileToSend"], endAction= next["endAction"])

					if retCode ==0: # all ok?
						self.setRPIonline(piUIN)
						self.RPI[piUIN][u"lastMessage"] = time.time()
						continue
						...
						addBack.append(next)
				try: 	self.rpiQueues["data"][piUIN].task_done()
				except: pass
				if addBack !=[]:
					for next in addBack:
						self.rpiQueues["data"][piUIN].put(next)
				self.rpiQueues["reset"][piUIN] =False
		except Exception, e:
			self.indiLOG.log(40,"in Line {} has error={}".format(sys.exc_traceback.tb_lineno, e))
		self.rpiQueues["state"][piUIN] = "stopped"
		self.indiLOG.log(20, u"rpiUpdateThread stopping,  rpi: {}  state: stopped".format(piUIN) )
		return
...
####-------------------------------------------------------------------------####
	def execSshToRPI(self, pi,fileToSend="", endAction="repeatUntilFinished"):
		...
		try:
			...
			if self.decideMyLog(u"UpdateRPI"): self.indiLOG.log(10, fileToSend+u" rPi# {}\n{}".format(pi, cmd) )
			if batch ==u" ":
			...
			if len(ret[1]) > 0:
				self.indiLOG.log(20, fileToSend+" Pi# {}  {}".format(pi, ret.replace(u"\n\n", u"\n")) )

			if fileToSend.find(u"getStats") >-1: 
				try:
					ret1= ((ret[0].split(u"===fix==="))[-1])
					self.indiLOG.log(20, u"stats from rpi# {} \n===fix==={}".format(pi, (ret1).replace(u"\n\n", u"\n")) )
				except:
...
####-------------------------------------------------------------------------####
	def printConfig(self):
		...
		self.myLog( text = u"wifi OFF if ETH0             " + unicode(self.wifiEth),					   mType= u"pi configuration")
		...
		self.myLog( text = u"collect x secs bf snd        " + unicode(self.sendAfterSeconds), 				mType= u"pi configuration")
		...
		if True:
			self.myLog( text = u" ==========  defined beacons ==============", mType= "pi configuration")
			self.myLog( text = u"#  Beacon MAC        indigoName                 indigoId Status          type      txMin ignore sigDlt minSig     LastUp[s] ExpTime updDelay           created    lastStatusChange",
	...
	def printTCPIPstats(self,all="yes"):
		...
		if self.socketServer is not None or True:
			if all == "yes":
					...
					self.myLog( text = u"Stats for RPI-->INDIGO data transfers. Tracking started "+startDate+u". Report TX errors if time between errors is <"+ unicode(int(self.maxSocksErrorTime)/60)+u" Min", mType=	 u"pi TCPIP socket")
				...
				if all == "yes" or all==IPN:
					for name  in sorted(self.dataStats[u"data"][IPN].keys()):
						for type in sorted(self.dataStats[u"data"][IPN][name].keys()):
							...
							self.myLog( text = IPN.ljust(15)+u" "+name.ljust(12) +u" "+type.ljust(10)+u" " + dtFT+u" "+dtLT +u" "+ count+u" "+ bytes+ u" "+ bytesPerMsg+ u" "+ maxByt+ u" "+ bytesPerMin+ u" "+ msgsPerMin,mType=" ")
			if all == "yes" and totMsg >0: 
				...
				self.myLog( text = "total                                                                          "+ unicode(int(totMsg)).rjust(10)+ unicode(int(totBytes)).rjust(13)+ u" "+ bytesPerMsg+ u" "+ maxBytes+ u" "+ bytesPerMin+ u" "+ msgsPerMin,mType=" ")
				self.myLog( text = u" ===  Stats for RPI --> INDIGO data transfers ==  END total time measured: "+unicode(int(time.strftime(u"%d", time.gmtime(secMeasured)))-1)+" "+time.strftime(u"%H:%M:%S", time.gmtime(secMeasured))+"; min measured: %2d"%minMeasured, mType=	 u"pi TCPIP socket")
		return 
...
####-------------------------------------------------------------------------####
	def printUpdateStats(self,):
		...
		self.myLog( text = "===    measuring started at: " +startDate,mType="indigo update stats " )
		self.myLog( text = "updates: %10d"%self.dataStats[u"updates"][u"devs"]  +u";   updates/sec: %10.2f"%(self.dataStats[u"updates"][u"devs"]  /nSecs)+u";   updates/minute: %10.2f"%(self.dataStats[u"updates"][u"devs"]  /nMin), mType=  u"    device ")
		self.myLog( text = "updates: %10d"%self.dataStats[u"updates"][u"states"]+u";   updates/sec: %10.2f"%(self.dataStats[u"updates"][u"states"]/nSecs)+u";   updates/minute: %10.2f"%(self.dataStats[u"updates"][u"states"]/nMin), mType=  u"    states ")
		...
		self.myLog( text = "updates: "+out, mType=  u"    #states")
		self.myLog( text = "===	total time measured: " +time.strftime(u"%H:%M:%S", time.gmtime(nSecs)), mType= "indigo update stats" )
		return 
