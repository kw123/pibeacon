...
def checkIndigoPluginName(self, indigo):
	"""Verifies that the plugin is installed under the correct .indigoPlugin folder name; if not, logs extensive error and remediation instructions, sleeps, and flags the plugin to quit.

	Inputs:
	    indigo (object): The Indigo server API module used for logging
	Outputs:
	    bool: True if the plugin name/path is correct, False (and sets self.quitNOW) otherwise
	"""
