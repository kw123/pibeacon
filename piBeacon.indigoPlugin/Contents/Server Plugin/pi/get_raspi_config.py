...
def execRaspi(params):
	...
	try:
		if logFile != "":
			logging.basicConfig(level=logging.INFO, filename= logFile,format='%(asctime)s %(module)-17s %(funcName)-22s L:%(lineno)-4d Lv:%(levelno)s %(message)s', datefmt='%Y-%m-%d-%H:%M:%S')
