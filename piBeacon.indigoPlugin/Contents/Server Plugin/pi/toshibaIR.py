...
#     two-frame burst (~90 kB) is over the limit and pigpiod ANSWERS BY CLOSING THE SOCKET: the
#     client sees "ConnectionResetError 104" inside wave_add_generic, and every later command on
#     that dead socket dies with "BrokenPipeError 32" somewhere else entirely.
#
#   NEVER SPLIT ONE WAVE ACROSS SEVERAL wave_add_generic CALLS. This was the first fix for the
#     size limit and it was WRONG: pigpio builds a wave by ABSOLUTE TIME OFFSET, which is what
#     lets you lay independent signals for different gpios into the same wave. A second call does
#     not continue where the first ended - it starts again at time 0 and MERGES. Chunking a single
#     burst therefore ORed frame 2 on top of frame 1: carrier where the gaps belong, marks running
#     together, spaces gone. Measured with a receiver: one frame (3767 pulses, one call) decodes
#     perfectly, two frames (7535 pulses, two calls) come back as mush - and every real command
#     sends two, which is why the AC ignored frames whose bytes were provably correct.
#
#   So: ONE FRAME = ONE WAVE = ONE CALL. A frame is ~3767 pulses = 45 kB, comfortably inside both
#     the socket limit and pigpiod's 12000 pulses per wave. The inter-frame gap is appended to the
#     wave that precedes it, so it stays DMA-timed instead of being slept in software.
MAXSOCKETPULSES	= 5400		# per wave_add_generic call: 5461 is the hard limit, leave a margin
...
def sendFrame(pigpioHandle, pin, frame, repeats=REPEATS, carrierHz=CARRIERHZ, dutyCycle=0.5, gap=GAP):
	...
	def sendPulses(pulseList):
		"""Builds ONE wave from pulseList in ONE call and runs it to the end. Blocks until done."""
		if len(pulseList) > MAXSOCKETPULSES:
			# splitting is NOT an option - see the note at MAXSOCKETPULSES - so say so instead of
			# silently sending a merged mess
			raise IOError("toshibaIR: {} pulses is more than one wave can take ({}); send fewer repeats".format(
							len(pulseList), MAXSOCKETPULSES))
		pigpioHandle.wave_add_new()
		added = pigpioHandle.wave_add_generic(pulseList)
		if added < 0:
			raise IOError("toshibaIR: pigpio refused {} pulses, error:{}".format(len(pulseList), added))
	...
	start = time.time()
	try:
		# ONE FRAME PER WAVE, always. The gap rides at the end of every frame but the last, so it
		# is DMA timed like everything else; the only software delay is the few ms it takes to
		# build and upload the next wave, which lands INSIDE nothing - the frame is complete.
		for n in range(nRepeats):
			if n < nRepeats - 1:	sendPulses(oneFrame + gapPulse)
			else:					sendPulses(oneFrame)
	finally:
