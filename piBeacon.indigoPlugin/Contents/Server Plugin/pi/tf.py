...
# imported ONCE, here, not per command: the first IR command should not be the one that pays for
# loading the module. Guarded because receiveCommands must keep running every other output type
# even if this one file did not make it onto the rpi.
try:
	import toshibaIR
except Exception:
	toshibaIR = None
...
gpioOwnerWhat	= {}	# pin -> what that call was, so a handover can NAME the successor. Without
						# it the log said only "a newer command owns the pin now", which is the one
						# question you actually have at that moment: newer command from WHERE
gpioOwnerLock	= threading.Lock()

PIGPIOhandle	= None	# ONE shared pigpio client for the whole process, see getPigpio()

# pigpiod has ONE waveform workspace: wave_add_new / wave_add_generic / wave_create are process
# wide, not per client. Two IR bursts built at the same time would concatenate into one garbage
# waveform, so the whole build+send is serialised. A burst is ~0.2 s, nothing waits long.
toshibaIRLock	= threading.Lock()

mapCmds	= {"pu":"pulseUp","pd":"pulseDown","cup":"continuousUpDown","aw":"analogWrite"}
####-------------------------------------------------------------------------####
def claimGPIOpin(pin, what=""):
	...
	Inputs:
	    ...
	    what (str): what is claiming it - named back at the handover, so "a newer command owns
	                the pin" says WHICH
	Outputs:
...
def getPigpio():
	...
	".connected" is NOT enough to decide that: it says what the client BELIEVES, and it stays True
	when pigpiod was restarted (or killed) after the client was built. The command socket is dead by
	then, and the next call raises BrokenPipeError deep inside pigpio - live-seen on a toshibaIR
	send, where the traceback names set_mode and says nothing about pigpiod. So the handle is
	verified with a real round trip on that socket; it costs one local socket exchange per command,
	which is nothing next to the command itself.
	...
	try:
		...
		if PIGPIOhandle is not None:
			try:
				if PIGPIOhandle.connected:
					PIGPIOhandle.get_pigpio_version()	# the honest test - raises when the pipe is gone
					return PIGPIOhandle
			except Exception as e:
				U.logger.log(20,"getPigpio: the pigpio connection is dead ({}) - pigpiod was restarted, reconnecting".format(e))
		...
		if not PIGPIOhandle.connected:
			...
			PIGPIOhandle = None
			# the daemon itself is gone, not just our socket. master starts it, but a command that
			# arrives before that (or after a pigpiod crash) would fail for no visible reason.
			if not U.pigpiodRunning(force=True):
				U.logger.log(20,"getPigpio: pigpiod is not running - starting it")
				subprocess.call("/usr/bin/sudo /usr/bin/pigpiod &", shell=True)
				time.sleep(2)
				U.pigpiodRunning(force=True)
				PIGPIOhandle = pigpio.pi()
				if not PIGPIOhandle.connected:
					try:	PIGPIOhandle.stop()
					except Exception:	pass
...
####-------------------------------------------------------------------------####
def dropPigpio():
	"""Throws the shared pigpio client away, so the next getPigpio() builds a fresh one.

	For the caller that has just SEEN the connection break: getPigpio() verifies the handle before
	handing it out, but pigpiod can still die in the window between that check and the command.
...
def setGPIO(command):
	...
	for iiii in range(1):
		...
		myGen = claimGPIOpin(pin, "setGPIO {} dev:{}".format(
					command.get("cmd", "?") if isinstance(command, dict) else command,
					command.get("devId", "?") if isinstance(command, dict) else "?"))
...
### ----------------------------------------- ###
def runIrRecord(params):
	"""Runs pi/irRecord.py for the plugin menu and sends its report back to indigo.

	Nothing has to be paused for this one - a TSOP receiver on its own gpio touches no radio and no
	other program, so unlike qualifyDongle it just runs. It BLOCKS for up to waitSecs waiting for a
	button press, which is why it gets its own thread.

	Inputs:
	    params (str): "gpio;waitSecs;count;ledGpio;duty" as built by the menu. ledGpio 0
	                  means record only; anything else turns on ECHO MODE, where each recording
	                  is retransmitted on that led - the test that says whether an ac ignoring
	                  our frames is a frame problem or an emitter problem
	Outputs:
	    None
	"""
	try:
		pp    = "{}".format(params).split(";")
		gpio  = pp[0].strip() if len(pp) > 0 and pp[0].strip() != "" else "24"
		secs  = pp[1].strip() if len(pp) > 1 and pp[1].strip() != "" else "15"
		count = pp[2].strip() if len(pp) > 2 and pp[2].strip() != "" else "1"
		led   = pp[3].strip() if len(pp) > 3 and pp[3].strip() != "" else "0"
		duty  = pp[4].strip() if len(pp) > 4 and pp[4].strip() != "" else "0.5"
		outFile = G.homeDir + "temp/irRecord.out"

		# NO sudo: irRecord only talks to pigpiod over its socket, and killSudos (master, every
		# 10th loop) kills anything whose command line holds both "python" and "sudo". An echo run
		# lasts about a minute, so with sudo it was reliably killed halfway - and killing the
		# wrapper only orphans the python child, so the report never came back while the pin
		# stayed claimed
		cmd = "python3 -u {}irRecord.py {} {} {} {} {}".format(G.homeDir, gpio, secs, count, led, duty)
		U.logger.log(20, "irRecord: running: {} > {} 2>&1".format(cmd, outFile))
		subprocess.call("{} > {} 2>&1".format(cmd, outFile), shell=True)
		...
		if out.strip() != "":
			U.logger.log(20, "irRecord: finished, sending {} chars of report back to indigo".format(len(out)))
			U.sendURL(data={"data": {"irRecordReport": out}}, squeeze=False, wait=False)
		else:
			U.logger.log(20, "irRecord: no output produced - nothing sent to indigo")
	except Exception:
		U.logger.log(20, "", exc_info=True)
	return
...
def execCMDS(nextItem):
	...
	for ijji in range(1):
			...
			if	cmd == "irRecord":
				# blocks for up to waitSecs waiting for a remote: own thread, like qualifyDongle
				try:
					U.logger.log(20, "execcmd. irRecord RECEIVED, params:>{}<".format(nextItem.get("device", "")))
					thR = threading.Thread(target=runIrRecord, args=("{}".format(nextItem.get("device", "")),))
					thR.daemon = True
					thR.start()
				except Exception:
					U.logger.log(20, "", exc_info=True)
			...
			if	cmd in ("irAC", "toshibaIR"):		# "toshibaIR" was the name before the device
														# became brand-independent; kept so an rpi
														# that still has an older plugin talking to
														# it does not go silent
				# Toshiba AC remote. ONE frame carries mode + fan + setpoint together and there is
				# no separate on/off command - "off" IS a mode - so the plugin always sends the
				# COMPLETE state, never a delta. Nothing is read back: IR is one way, the plugin
				# owns the state.
				if delayStart > 0 and delayStart < 10000000:
					U.logger.log(DEBUG,"{:.2f} delay start by: {}".format(time.time(), delayStart))
					if sleepForxSecs(delayStart):
				...
				myGen	= -1
				pin		= -1
				if toshibaIR is None:
					U.logger.log(20, "execcmd. toshibaIR: module toshibaIR.py is not on this rpi, nothing sent - send the plugin files to the rpis")
					continue
				try:
					vals	= nextItem.get("values", {})
					myGen	= 0		# the finally below reads it, and every "continue" here reaches that
					try:	pin = int(nextItem.get("pin", -1))
					except Exception:	pin = -1
					if pin < 0:
						U.logger.log(20, "execcmd. toshibaIR: no/bad gpio pin in command, nothing sent: {}".format(nextItem))
						continue

					# the settings are pulled out FIRST, defaults and all, so the log line below can
					# report what was actually encoded. The AC never answers, so this log is the only
					# record of what was asked for - the frame hex alone does not say it in words.
					irCommand	= "{}".format(vals.get("irCommand", "modeFanTemp"))
					unit		= "{}".format(vals.get("unit", "a"))
					# carrier duty: 0.5 is the textbook value, real remotes send about 1/3. Read
					# HERE, at the TOP, before any branch: every branch below - toshiba, gree and
					# the led test - reads it, and a later definition leaves it unbound on the
					# paths that run first. That has now bitten twice, both times as
					# "UnboundLocalError: duty" from a different branch.
					try:	duty = float(vals.get("duty", 0.5))
					except Exception:	duty = 0.5
					if duty < 0.1 or duty > 0.9:	duty = 0.5
					# WHICH PROTOCOL. The device is an IR ac remote and the brand is a setting; each
					# brand is its own encoder module. Anything but toshiba has no encoder yet - say
					# which one was asked for instead of silently sending a toshiba frame to it.
					brand		= "{}".format(vals.get("brand", "toshiba")).strip().lower()
					if brand == "":	brand = "toshiba"
					if brand not in ("toshiba", "gree"):
						U.logger.log(20, "execcmd. IR-ac: brand '{}' has no encoder on this rpi - nothing sent. Record its remote (menu: Record an IR remote) and the protocol can be added".format(brand))
						continue
					if brand == "gree" and greeIR is None:
						U.logger.log(20, "execcmd. IR-ac: greeIR.py is not on this rpi - send the plugin files to the rpis")
						continue

					# GREE: a different encoder and a different frame, the same led and the same
					# pin claim. Power is its own bit, so "off" is power=False and the mode stays.
					if brand == "gree" and irCommand not in ("test",):
						greeMode	= "{}".format(vals.get("mode", "cool"))
						greePower	= (greeMode != "off")
						if not greePower:	greeMode = "{}".format(vals.get("modeWhenOff", "cool"))
						state = greeIR.buildState(
									mode		= greeMode,
									temperature	= vals.get("temperature", 22),
									fan			= "{}".format(vals.get("fan", "auto")),
									power		= greePower,
									swingV		= "{}".format(vals.get("swingV", "last")),
									swingH		= "{}".format(vals.get("swingH", "off")),
									light		= int(vals.get("light", 1)),
									health		= int(vals.get("health", 0)),
									sleep		= int(vals.get("sleep", 0)),
									swingAuto	= int(vals.get("swingAuto", 0)),
									turbo		= int(vals.get("turbo", 0)),
									xfan		= int(vals.get("xfan", 0)),
									econo		= int(vals.get("econo", 0)),
									)
						try:	greeRepeats = int(vals.get("repeats", greeIR.REPEATS))
						except Exception:	greeRepeats = greeIR.REPEATS
						myGen	= claimGPIOpin(pin, "irAC gree")
						handle	= getPigpio()
						if handle is None:
							U.logger.log(20, "execcmd. IR-ac gree: pigpiod not reachable - nothing sent")
							continue
						if gpioPinTakenOver(pin, myGen):
							U.logger.log(20, "execcmd. IR-ac gree: gpio {} was taken over before the burst started, nothing sent".format(pin))
							continue
						with toshibaIRLock:				# ONE ir lock: pigpiod has one wave workspace
							try:
								took = greeIR.sendSequence(handle, pin, state, fan="{}".format(vals.get("fan", "auto")), dutyCycle=duty)
							except (IOError, OSError) as e:
								U.logger.log(20, "execcmd. IR-ac gree: pigpio error '{}' - reconnecting and sending once more".format(e))
								dropPigpio()
								handle = getPigpio()
								if handle is None:
					...
					# LED TEST: no frame at all, just the carrier on and off in whole seconds so the
					# led can be SEEN blinking in a phone camera. Same path as a real command - socket,
					# pin claim, pigpio, transistor, led - so it proves the whole chain, not the encoder.
					if irCommand == "test":
						try:	testSecs = float(vals.get("seconds", 1.))
						except Exception:	testSecs = 1.
						try:	testCycles = int(vals.get("cycles", 2))
						except Exception:	testCycles = 2
						myGen	= claimGPIOpin(pin, "irAC ledTest")
						handle	= getPigpio()
						if handle is None:
							U.logger.log(20, "execcmd. IR-ac led test: pigpiod not reachable, IR needs it for the 38kHz carrier - nothing sent")
							continue
						if gpioPinTakenOver(pin, myGen):
							U.logger.log(20, "execcmd. IR-ac led test: gpio {} was taken over before the test started, nothing sent".format(pin))
							continue
						U.logger.log(20, "execcmd. IR-ac led TEST on gpio:{}: {} x (carrier {:.1f}s on, {:.1f}s off) - watch the led in a phone camera".format(
											pin, testCycles, testSecs, testSecs))
						with toshibaIRLock:
							try:
								took = toshibaIR.testBurst(handle, pin, seconds=testSecs, cycles=testCycles, dutyCycle=duty)
							except (IOError, OSError) as e:
								U.logger.log(20, "execcmd. IR-ac led test: pigpio error '{}' - reconnecting to pigpiod and trying once more".format(e))
								dropPigpio()
								handle = getPigpio()
								if handle is None:
									U.logger.log(20, "execcmd. IR-ac led test: pigpiod not reachable after the reconnect - nothing sent")
									continue
								took = toshibaIR.testBurst(handle, pin, seconds=testSecs, cycles=testCycles, dutyCycle=duty)
						U.logger.log(20, "execcmd. IR-ac led TEST done on gpio:{} in {:.3f}s - if the led did not blink the fault is in the pin, the transistor or the led, not in the AC".format(pin, took))
						continue

					if	 irCommand == "swing":
						frame	= toshibaIR.swing(unit=unit)
						asked	= "unit:{}".format(unit)
					elif irCommand == "fix":
						frame	= toshibaIR.fix(unit=unit)
						asked	= "unit:{}".format(unit)
					else:
						mode		= "{}".format(vals.get("mode", "cool"))
						temperature	= vals.get("temperature", 22)
						fan			= "{}".format(vals.get("fan", "auto"))
						special		= "{}".format(vals.get("special", "none"))
						msgLength	= "{}".format(vals.get("length", "normal"))
						byte7Style	= "{}".format(vals.get("byte7", "remote"))
						...
						asked	= "{} mode:{} temp:{} fan:{} special:{} unit:{} msg:{}({} bytes) byte7:{} duty:{:.2f}".format(
									brand if brand != "" else "toshiba",
									mode, temperature, fan, special, unit, msgLength, len(frame), byte7Style, duty)

					try:	repeats = int(vals.get("repeats", toshibaIR.REPEATS))
					except Exception:	repeats = toshibaIR.REPEATS
					try:	gap = int(vals.get("gap", toshibaIR.GAP))
					except Exception:	gap = toshibaIR.GAP

					myGen	= claimGPIOpin(pin, "irAC toshiba")
					handle	= getPigpio()
					if handle is None:
						U.logger.log(20, "execcmd. toshibaIR: pigpiod not reachable, IR needs it for the 38kHz carrier - nothing sent")
						continue
					if gpioPinTakenOver(pin, myGen):
						U.logger.log(20, "execcmd. toshibaIR: gpio {} was taken over before the burst started, nothing sent".format(pin))
						continue

					with toshibaIRLock:
						try:
							took = toshibaIR.sendFrame(handle, pin, frame, repeats=repeats, gap=gap, dutyCycle=duty)
						except (IOError, OSError) as e:
							# pigpiod can die (or be restarted) between the check in getPigpio() and
							# this call - then set_mode/wave_* raise BrokenPipeError. The frame is
							# built from scratch inside sendFrame, so retrying it is safe.
							U.logger.log(20, "execcmd. toshibaIR: pigpio error '{}' - reconnecting to pigpiod and sending once more".format(e))
							dropPigpio()
							handle = getPigpio()
							if handle is None:
								U.logger.log(20, "execcmd. toshibaIR: pigpiod not reachable after the reconnect - nothing sent")
								continue
							took = toshibaIR.sendFrame(handle, pin, frame, repeats=repeats, gap=gap, dutyCycle=duty)

					# level 20 on purpose: with no readback from the AC, this log line is the only
					# evidence the frame ever left the rpi - and the only record of WHAT was sent
					U.logger.log(20, "execcmd. toshibaIR sent on gpio:{}  {}: {}  frame:{} x{} in {:.3f}s".format(
										pin, irCommand, asked, toshibaIR.frameToHex(frame), repeats, took))

				except Exception:
					U.logger.log(20,"", exc_info=True)
				finally:
					if myGen > 0 and pin >= 0:
						ownsGPIOpin(pin, myGen)		# drop the claim, the burst is over
				continue
