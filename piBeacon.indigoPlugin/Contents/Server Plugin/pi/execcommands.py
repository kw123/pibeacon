...
def execCMDS(data):
	...
	#U.logger.log(debLevel, u"{:.2f} into execCMDS".format(time.time()))
	...
	for ijji in range(1):
			...
			U.logger.log(debLevel,"{:.2f} next command: {}".format(time.time(), data))
			cmd= next["command"]
			...
			if device=="OUTgpio" or device.find("OUTPUTgpio")> -1:
						...
						if not externalGPIO: # this takes 0.15 secs 
						...
						if externalGPIO:
							...
							#U.logger.log(10,"cmd= {}".format(cmdOut) )
							subprocess.call(cmdOut, shell=True)
						else:
							#U.logger.log(20, "{:.2f} bf setGPIO curr_pid={},  command:{}".format(time.time(), myPID,cmdJD) )
							setGPIO(cmdJ)
						continue
...
if True: #__name__ == "__main__":
	debLevel = 20
	#print( u"{:.2f} into exec cmd".format(time.time()) )
	PWM = 100
	myPID = int(os.getpid())
	U.setLogging()
	readParams()
...
#### read exec command list for restart values, update if needed and write back
	...
	#U.logger.log(debLevel, u"{:.2f} exec cmd received: {:}".format(time.time(),sys.argv))
