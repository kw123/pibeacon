...
def get18B20(sensor, data):
	...
	try:
		data[sensor]={}
		try:
			devs=readPopen("cat /sys/bus/w1/devices/w1_bus_master1/w1_master_slaves")[0].split("\n")
			#U.logger.log(20, " getting data devs={}<< ".format(devs))
			ret = {}
			for line in devs:
				if len(line) < 5: continue
				if line.find("28-")==-1: continue  # should be something like: "28-800000035de5"
				#U.logger.log(20,"wire18B20 return data1 {}".format(line))
				if not os.path.isfile("/sys/bus/w1/devices/"+line+"/w1_slave"): continue
				oneWirecmd="cat  /sys/bus/w1/devices/"+line+"/w1_slave"
				dataW= readPopen(oneWirecmd)[0].strip("\n").split("\n")
				U.logger.log(20,"wire18B20 return data2 {}".format(dataW))
...
def readParams():
		...
		if sensorList.find("Wire18B20") == -1:
			U.logger.log(30," Wire18B20 not defined in parameters, exiting  ")
			exit()
