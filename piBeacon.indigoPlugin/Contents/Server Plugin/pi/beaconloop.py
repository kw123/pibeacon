				...
				for sensor in sensors:
					for devId in sensors[sensor]:
						if "isBLESensorDevice" in sensors[sensor][devId] and sensors[sensor][devId]["isBLESensorDevice"]:
							...
							try:	BLEsensorMACs[mac][sensor]["multiplyTemp"] 			= float(sensD["multiplyTemp"])
							except: BLEsensorMACs[mac][sensor]["multiplyTemp"] 			= 1.
							...
							if "accelerationTotal" not in BLEsensorMACs[mac][sensor]:
								...
								BLEsensorMACs[mac][sensor]["lastUpdate"]				 	= 0
...
def doSensors( mac, macplain, macplainReverse, rx, tx, hexData, UUID, Maj, Min):
	...
	try:
		...
		for sensor in BLEsensorMACs[mac]:
			...
			if sensor.find("BLEMiTempHumRound") >-1:
				return  doBLEMiTempHumRound( mac, macplain, macplainReverse, rx, tx, hexData, UUID, Maj, Min, sensor)
...
def doBLEMiTempHumRound(mac, macplain, macplainReverse, rx, tx, hexData, UUID, Maj, Min, sensor):
...
def doBLEiBSxx( mac, macplain, macplainReverse, rx, tx, hexData, UUID, Maj, Min, sensor):
	...
	try:
		...
		if  sensor  == "BLEiBS01" : 	# on/off
			...
			p = 26 # start of temp
			temp = ( signedIntfrom16( HexStr[p+2:p+4] + HexStr[p:p+2] )/100. + BLEsensorMACs[mac][sensor]["offsetTemp"]) * BLEsensorMACs[mac][sensor]["multiplyTemp"]
...
def batLevelTempCorrection(batteryVoltage, temp):
	try:
		battreyLowVsTemp			= (1. + 0.7*min(0.,temp-10.)/100.) * 2700. # (changes to 0.9* 2700 @ 0C; to = 0.8*2700 @-10C )
		batteryLevel 				= int(min(100.,max(0.,100.* (batteryVoltage - battreyLowVsTemp)/(3000.-battreyLowVsTemp))))
		return batteryLevel
	except	Exception, e:
		U.logger.log(30,u"in Line {} has error={}".format(sys.exc_traceback.tb_lineno, e))
	return 0
...
def domyBlueT( mac, rx, tx, hexData, UUID, Maj, Min,sensor):
	...
	try:
		...
		sensor			= "BLEmyBlueT"
		devId			= BLEsensorMACs[mac][sensor]["devId"]
		try:	temp  	= (sensorData + BLEsensorMACs[mac][sensor]["offsetTemp"]) * BLEsensorMACs[mac][sensor]["multiplyTemp"]
...
18-23	Any valid mac		48bit MAC address, should be same as in message  (==mac)
	...
	try:
		...
		temp 					= (doRuuviTag_temperature(byte_data)+ BLEsensorMACs[mac][sensor]["offsetTemp"]) * BLEsensorMACs[mac][sensor]["multiplyTemp"]
...
def checkIFtrackMacIsRequested():
	...
	try:
		...
		startTimeTrackMac = time.time() + collectTime
...
def writeTrackMac(textOut0, textOut2, mac):
	...
	try:
		...
		if trackRawOnly and textOut0.find("RAW") == -1: return 
	...
	try:
		...
		## 
			...
			for line in xxx.split("\n"):
				...
				if mac not in MACs: 
					MACs[mac] = {"max_rssi":-99, "max_TX": -99,"MSG_in_10Secs": 0,"mfg_info":[],"TLM":[],"iBeacon":[],"n_of_MSG_Types":0,"typeOfBeacon":[],"typeOfBeacon-msg#":[],"raw_data":[],"pos_of_MAC_in_UUID":[],"pos_of_reverse_MAC_in_UUID":[], "possible_knownTag_options":[]}
				present = False
				...
				if not present:
					...
					if "mfg_info" in parsedData:
						MACs[mac]["mfg_info"].append(parsedData["mfg_info"])
					else:
						MACs[mac]["mfg_info"].append("")

					if "TLM" in parsedData:
						MACs[mac]["TLM"].append(parsedData["TLM"])
					else:
						MACs[mac]["TLM"].append("")
					
					if "iBeacon" in parsedData:
						MACs[mac]["iBeacon"].append(parsedData["iBeacon"])
					else:
						MACs[mac]["iBeacon"].append("")
...
def getTLMdata(mac, res, verbose = False):
	try:
		...
		if l >-1:
			...
			if  len(res) - l == 32:
				k = l+6
				VbatH 		= res[k+2:k+4]+res[k+0:k+2]
				Vbat 		= int(res[k+2:k+4]+res[k+0:k+2],16)
				k += 4
				tempH 		= res[k+2:k+4]+res[k+0:k+2]
				...
				advCountH 	= res[k+0:k+2]+res[k+2:k+4]+res[k+4:k+6]+res[k+6:k+8]
				advCount 	= int(advCountH,16)
				k += 8
				timeSinceH 	= res[k+6:k+8]+res[k+4:k+6]+res[k+2:k+4]+res[k+0:k+2]
				timeSince 	= int(timeSinceH,16)/10.
				retData = {"batteryVoltage":Vbat, "temp":temp, "advCount":advCount, "timeSince":timeSince}
				if verbose: 
					U.logger.log(20,u" mac:{:}, Vbat:{:}={:4d}; temp:{:}={:.2f}, advCount:{:}={:10d}, timeSince:{:}={:12.1f}".format(mac, VbatH, Vbat, tempH, temp, advCountH, advCount, timeSinceH, timeSince))
